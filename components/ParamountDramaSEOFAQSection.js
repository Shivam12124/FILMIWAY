// components/ParamountDramaSEOFAQSection.js - MATCHING PARAMOUNT+ DRAMA DESIGN (Gold/Indigo Theme) ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info } from 'lucide-react';
import { PARAMOUNT_DRAMA_MOVIE_FAQS } from '../utils/paramountDramaMovieData';

const ParamountDramaSEOFAQSection = ({ movie }) => {
    // 🔥 Get FAQs from PARAMOUNT_DRAMA_MOVIE_FAQS data
    const faqsFromData = movie?.Title && PARAMOUNT_DRAMA_MOVIE_FAQS?.[movie.Title] 
        ? PARAMOUNT_DRAMA_MOVIE_FAQS[movie.Title] 
        : [];

    // 🔥 Safety check - return null if no FAQs
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No Paramount+ Drama FAQs found for:', movie?.Title);
        return null;
    }
