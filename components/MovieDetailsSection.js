// components/MovieDetailsSection.js - WITH SURVIVAL FAQ SUPPORT ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Users, Film, BookOpen, Star, Brain, BarChart3, Shield } from 'lucide-react';

// 🔥 FIXED: Added SENSITIVE_TIMELINES to imports
...
const MovieDetailsSection = React.memo(({ movie, fromMementoCollection, fromShutterIslandCollection, fromInceptionCollection, fromSurvivalCollection }) => {
  ...
  const getMovieSpecificData = (title) => {
    ...
    if (t.includes('shutter island')) {
      return {
        mindBendingIndex: 79, complexityLevel: 'MEDIUM', dominantColor: '#dc2626', rating: 8.2, director: 'Martin Scorsese',
        scenes: [
        ...
        boxOffice: '$294.8M', budget: '$80M'
      };
    }
    
    if (t.includes('usual suspects')) {
      return {
        mindBendingIndex: 88, complexityLevel: 'EXTREME', dominantColor: '#ea580c', rating: 8.5, director: 'Bryan Singer',
        scenes: [
        ...
        boxOffice: '$23.3M', budget: '$6M'
      };
    }
    
    if (t.includes('memento')) {
      return {
        mindBendingIndex: 95, complexityLevel: 'EXTREME', dominantColor: '#7c3aed', rating: 8.4, director: 'Christopher Nolan',
        scenes: [
        ...
        boxOffice: '$40.0M', budget: '$9M'
      };
    }
    
    if (t.includes('inception')) {
      return {
        mindBendingIndex: 93, complexityLevel: 'EXTREME', dominantColor: '#0ea5e9', rating: 8.8, director: 'Christopher Nolan',
        scenes: [
        ...
        boxOffice: '$836.8M', budget: '$160M'
      };
    }

    // 🔥 NEW: Inception collection movies
    if (t.includes('enemy')) {
      return {
        mindBendingIndex: 87, complexityLevel: 'EXTREME', dominantColor: '#eab308', rating: 6.9, director: 'Denis Villeneuve',
        scenes: [
          { time: 10, intensity: 30, label: 'Doppelganger Discovery', color: '#92400e' },
          { time: 30, intensity: 50, label: 'First Contact', color: '#ca8a04' },
          { time: 50, intensity: 70, label: 'Identity Crisis', color: '#eab308' },
          { time: 70, intensity: 85, label: 'Confrontation', color: '#facc15' },
          { time: 90, intensity: 100, label: 'Spider Revelation', color: '#fde047' }
        ],
        dna: { "Psychological": 80, "Mystery": 15, "Thriller": 5 },
        cast: ['Jake Gyllenhaal', 'Mélanie Laurent', 'Sarah Gadon'],
        boxOffice: '$3.4M', budget: '$9M'
      };
    }

    if (t.includes('primer')) {
      return {
        mindBendingIndex: 96, complexityLevel: 'EXTREME', dominantColor: '#8b5cf6', rating: 6.9, director: 'Shane Carruth',
        scenes: [
          { time: 15, intensity: 40, label: 'Discovery', color: '#581c87' },
          { time: 30, intensity: 60, label: 'First Test', color: '#7c3aed' },
          { time: 50, intensity: 75, label: 'Temporal Loop', color: '#8b5cf6' },
          { time: 65, intensity: 90, label: 'Timeline Fracture', color: '#a78bfa' },
          { time: 77, intensity: 100, label: 'Infinite Recursion', color: '#c4b5fd' }
        ],
        dna: { "Sci-Fi": 70, "Thriller": 20, "Mystery": 10 },
        cast: ['Shane Carruth', 'David Sullivan', 'Casey Gooden'],
        boxOffice: '$0.4M', budget: '$0.007M'
      };
    }

    if (t.includes('coherence')) {
      return {
        mindBendingIndex: 89, complexityLevel: 'EXTREME', dominantColor: '#06b6d4', rating: 7.2, director: 'James Ward Byrkit',
        scenes: [
          { time: 10, intensity: 30, label: 'Dinner Party', color: '#0c4a6e' },
          { time: 25, intensity: 55, label: 'Power Outage', color: '#0891b2' },
          { time: 45, intensity: 70, label: 'Parallel House', color: '#06b6d4' },
          { time: 65, intensity: 85, label: 'Reality Splits', color: '#67e8f9' },
          { time: 89, intensity: 100, label: 'Infinite Versions', color: '#a7f3d0' }
        ],
        dna: { "Sci-Fi": 60, "Mystery": 25, "Thriller": 15 },
        cast: ['Emily Baldoni', 'Maury Sterling', 'Nicholas Brendon'],
        boxOffice: '$0.1M', budget: '$0.05M'
      };
    }

    if (t.includes('donnie darko')) {
      return {
        mindBendingIndex: 84, complexityLevel: 'HIGH', dominantColor: '#dc2626', rating: 8.0, director: 'Richard Kelly',
        scenes: [
          { time: 15, intensity: 35, label: 'Frank Appears', color: '#7c2d12' },
          { time: 40, intensity: 55, label: 'Time Portal', color: '#dc2626' },
          { time: 70, intensity: 75, label: 'Philosophy of Time Travel', color: '#ef4444' },
          { time: 95, intensity: 85, label: 'Engine Falls', color: '#f87171' },
          { time: 113, intensity: 100, label: 'Timeline Reset', color: '#fca5a5' }
        ],
        dna: { "Sci-Fi": 50, "Mystery": 30, "Drama": 20 },
        cast: ['Jake Gyllenhaal', 'Jena Malone', 'Mary McDonnell'],
        boxOffice: '$7.5M', budget: '$4.5M'
      };
    }
  ...
  // 🔥 UPDATED: Use appropriate index based on collection
  ...
  const getComplexityColor = (level) => {
    // 🔥 UPDATED: Add Inception collection colors (Yellow theme)
  ...
  const getUniqueDescription = () => {
    ...
    // 🏔️ For survival movies, use the synopsis from data FIRST
    if (fromSurvivalCollection && safeMovieInfo?.synopsis) {
      return safeMovieInfo.synopsis;
    }

    // 🔥 NEW: Inception collection descriptions
  ...
  const getComplexityDescription = () => {
    if (fromSurvivalCollection) {
      if (displayIndex >= 90) return "An extraordinary survival experience that pushes human endurance to absolute limits, showcasing unbreakable determination.";
      if (displayIndex >= 80) return "Intense survival narrative with brutal challenges testing physical and mental resilience throughout.";
      if (displayIndex >= 70) return "Compelling survival story with engaging life-or-death stakes and authentic human perseverance.";
      return "Accessible survival drama with genuine tension and relatable human struggle against the odds.";
    }
    
    // 🔥 NEW: Inception collection descriptions
    if (fromInceptionCollection) {
      if (displayIndex >= 90) return "A transcendent masterpiece that redefines reality-bending storytelling, requiring multiple viewings to fully grasp the layered complexity.";
      if (displayIndex >= 80) return "Sophisticated mind-bending narrative with advanced reality-questioning elements and identity complexity throughout.";
      if (displayIndex >= 70) return "Thoughtfully complex reality-bending story with engaging perception-altering elements and philosophical depth.";
      return "Accessible mind-bending complexity with subtle reality-questioning elements that reward careful viewing.";
    }
    
    if (fromShutterIslandCollection) {
      if (displayIndex >= 90) return "A transcendent masterpiece that redefines psychological storytelling, requiring multiple viewings for complete comprehension.";
      if (displayIndex >= 80) return "Sophisticated psychological manipulation with advanced reality-questioning elements and identity complexity.";
      if (displayIndex >= 70) return "Thoughtfully complex psychological narrative with engaging mind-bending elements throughout.";
    ...
    if (fromMementoCollection) {
      if (displayIndex >= 90) return "A transcendent masterpiece that redefines memory-based storytelling, requiring multiple viewings for complete comprehension.";
      if (displayIndex >= 80) return "Sophisticated memory manipulation with advanced identity-questioning elements and temporal complexity.";
      if (displayIndex >= 70) return "Thoughtfully complex memory narrative with engaging identity-bending elements throughout.";
      return "Accessible memory complexity with subtle identity-questioning elements that reward careful viewing.";
    }
    
    if (displayIndex >= 90) return "A transcendent masterpiece that redefines narrative complexity, requiring multiple viewings for complete comprehension.";
    if (displayIndex >= 80) return "Sophisticated cinematic storytelling with advanced non-linear elements and reality-bending concepts.";
    if (displayIndex >= 70) return "Thoughtfully complex narrative structure with engaging mind-bending elements throughout.";
  ...
  return (
    ...
    >
      ...
      >
        {/* 🔥 UPDATED: Use appropriate colors for each collection */}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4 sm:gap-6">
          ...
          <div className="flex items-center gap-6 sm:gap-8">
            <div className="text-right">
              <div className="text-3xl sm:text-4xl font-extralight mb-1 tracking-wider text-gray-200">{displayIndex}</div>
      ...
      {/* 🔥 UPDATED: Use appropriate FAQ section based on collection */}
