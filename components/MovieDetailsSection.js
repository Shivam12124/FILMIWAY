...
const MovieDetailsSection = React.memo(({
  ...
  const director = movie.Director || safeMovieInfo.director || 'Unknown Director';
  const runtime = (movie.runtime || movie.Runtime || safeMovieInfo.runtime)
    ...
    : 'Unknown';
  const ageRating = (movie.Rated && movie.Rated !== 'Rated') ? movie.Rated : (safeMovieInfo.ageRating || 'NR');
  ...
  return (
    ...
    >
      ...
      <div className="w-full min-h-[380px] sm:min-h-[320px] bg-[#0a0a0c] rounded-2xl border border-white/10 shadow-xl p-5 sm:p-8 clean-injected-component">
      ...
      <div className="w-full bg-[#0a0a0c] rounded-2xl border border-white/10 shadow-xl p-5 sm:p-8 faq-unified-container">
        ...
        `}</style>
        <SEOFAQSection movie={{ ...movie, Runtime: runtime, runtime: runtime }} />
      </div>
    </motion.div>
  );
});
