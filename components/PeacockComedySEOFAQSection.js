// components/PeacockComedySEOFAQSection.js - MATCHING INCEPTION DESIGN ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Smile } from 'lucide-react'; // 💛 Swapped to Smile for the Comedy theme
// 🔥 IMPORT THE NEW DYNAMIC FAQ GENERATOR
import { COMPLETE_MOVIE_DATA as PEACOCK_COMEDY_MOVIE_DATA, getVisibleMovieFAQs } from '../utils/peacockComedyMovieData';

const PeacockComedySEOFAQSection = ({ movie }) => {
    const movieInfo = PEACOCK_COMEDY_MOVIE_DATA[movie?.tmdbId];
    ...
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No Peacock Comedy FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base">
                Common questions about {movie?.Title}, including its biggest laughs, memorable quotes, and our parents guide.
            </p>
            <div className="space-y-4 sm:space-y-6">
                {faqsFromData.map((faq, index) => (
                    <motion.div 
                        key={index}
                        className="bg-gray-900/40 rounded-xl p-4 sm:p-6 border border-yellow-900/30 hover:border-yellow-600/50 transition-colors duration-300"
...
export default PeacockComedySEOFAQSection;