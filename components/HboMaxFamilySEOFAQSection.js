// components/HboMaxFamilySEOFAQSection.js - HBO MAX FAMILY THEME (YELLOW/AMBER) ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info, Sparkles } from 'lucide-react';
import { HBO_MAX_FAMILY_MOVIE_FAQS } from '../utils/hboMaxFamilyMovieData';

const HboMaxFamilySEOFAQSection = ({ movie }) => {
    // 🔥 Get FAQs from HBO_MAX_FAMILY_MOVIE_FAQS data
    const faqsFromData = movie?.Title && HBO_MAX_FAMILY_MOVIE_FAQS?.[movie.Title] 
        ? HBO_MAX_FAMILY_MOVIE_FAQS[movie.Title] 
        : [];

    // 🔥 Safety check - return null if no FAQs
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No HBO Max Family FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base">
                Discover more about the magic, themes, and production of {movie.Title}.
            </p>
