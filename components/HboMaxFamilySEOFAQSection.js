// components/HboMaxFamilySEOFAQSection.js - DYNAMIC & SEO OPTIMIZED ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Sparkles } from 'lucide-react'; // Matches the Magical/Whimsical vibe
// 🔥 IMPORT THE DYNAMIC FAQ GENERATOR
import { getVisibleMovieFAQs } from '../utils/hboMaxFamilyMovieData';

const HboMaxFamilySEOFAQSection = ({ movie }) => {
    ...
    return (
        ...
        >
            {/* 🎨 Magical Yellow/Amber Theme for the Family Collection */}
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base leading-relaxed">
                Explore our Parents Guide and expert analysis for {title}. We provide accurate timestamps for sensitive or frightening scenes to skip, and map the most exciting magical moments of this HBO Max family hit.
            </p>
            ...
            <div className="space-y-4 sm:space-y-6">
                {faqsFromData.map((faq, index) => (
                    <motion.div 
                        key={index}
                        className="bg-gray-800/30 rounded-xl p-4 sm:p-6 border border-gray-700/50 hover:border-yellow-500/30 transition-colors duration-300 group"
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ duration: 0.6, delay: index * 0.1 }}
                    >
                        {/* 🔥 The exact question (Static Trivia OR Dynamic Parents Guide) */}
                        <h3 className="text-base sm:text-lg font-medium text-yellow-300 mb-2 sm:mb-3 leading-relaxed">
                            {faq.question || faq.q}
                        </h3>
...
export default HboMaxFamilySEOFAQSection;