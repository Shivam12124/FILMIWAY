...
const Header = () => {
  ...
  return (
    <nav className="fixed top-0 w-full z-[99999] bg-black/80 backdrop-blur-xl select-none border-b border-white/5">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-14 sm:h-20 gap-3 sm:gap-6">
          ...
          {/* ⚡ GLOBAL SEARCH BAR / TAGLINE */}
          {isHomePage ? (
            <div className="flex-1 text-center hidden md:block mx-4">
              <span className="text-gray-400 text-[13px] font-normal tracking-[0.15em] uppercase hover:text-yellow-400/80 transition-colors duration-300 cursor-default">
                Check if any movie is safe to watch with family
              </span>
            </div>
          ) : (
            !isSearchPage && (
                          ...
                          return (
                            <li key={movie.imdbID}>
                              <Link 
                                ...
                                onClick={() => { setIsOpen(false); setSearchQuery(''); clearCollectionMemory(); }} 
