// components/HboMaxRomanceSEOFAQSection.js - MATCHING ROMANCE DESIGN ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info } from 'lucide-react';
// ✅ Pull in the unified UI generator instead of the raw static array
import { getVisibleMovieFAQs } from '../utils/hboMaxRomanceMovieData';
const HboMaxRomanceSEOFAQSection = ({ movie }) => {
    ...
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No HBO Max Romance FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base">
                Common questions about <strong>{movie.Title}</strong> and its place in the HBO Max romance collection.
            </p>
            <div className="space-y-4 sm:space-y-6">
                {faqsFromData.map((faq, index) => (
                    ...
                    >
                        <h3 className="text-base sm:text-lg font-medium text-rose-200 mb-2 sm:mb-3">
                            {faq.question}
                        </h3>
...
export default HboMaxRomanceSEOFAQSection;