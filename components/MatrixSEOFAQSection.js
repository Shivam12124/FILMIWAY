// components/MatrixSEOFAQSection.js - FIXED ✅
...
const MatrixSEOFAQSection = ({ movie }) => {
    const movieInfo = MATRIX_MOVIE_DATA[movie.tmdbId];
    
    // 🔥 Get FAQs from MATRIX_MOVIE_FAQS data
    const faqsFromData = movie?.Title && MATRIX_MOVIE_FAQS?.[movie.Title] 
        ? MATRIX_MOVIE_FAQS[movie.Title] 
        : [];

    // 🔥 Safety check - return null if no FAQs
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No Matrix FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <p className="text-emerald-200/80 mb-4 sm:mb-6 text-sm sm:text-base">
                Common questions about {movie.Title} and this reality-bending cyberpunk masterpiece.
            </p>
            <div className="space-y-4 sm:space-y-6">
                {faqsFromData.map((faq, index) => (
                    ...
                    >
                        <h3 className="text-base sm:text-lg font-medium text-emerald-300 mb-2 sm:mb-3">{faq.question}</h3>
