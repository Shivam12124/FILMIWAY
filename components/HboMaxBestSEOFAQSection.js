// components/HboMaxBestSEOFAQSection.js - MATCHING PRESTIGE GOLD DESIGN ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info, Award } from 'lucide-react';
import { HBO_BEST_MOVIE_FAQS } from '../utils/hboMaxBestMoviesData';

const HboMaxBestSEOFAQSection = ({ movie }) => {
    // 🔥 Get FAQs from HBO_BEST_MOVIE_FAQS data
    const faqsFromData = movie?.Title && HBO_BEST_MOVIE_FAQS?.[movie.Title] 
        ? HBO_BEST_MOVIE_FAQS[movie.Title] 
        : [];

    // 🔥 Safety check - return null if no FAQs
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No HBO Max Best FAQs found for:', movie?.Title);
        return null;
    }
