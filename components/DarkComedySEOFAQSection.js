// components/DarkComedySEOFAQSection.js - MATCHING INCEPTION DESIGN ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info } from 'lucide-react';
// 🔥 IMPORT THE NEW DYNAMIC FAQ GENERATOR
import { COMPLETE_MOVIE_DATA as DARK_COMEDY_MOVIE_DATA, getVisibleMovieFAQs } from '../utils/darkComedyMovieData';

const DarkComedySEOFAQSection = ({ movie }) => {
    const movieInfo = DARK_COMEDY_MOVIE_DATA[movie?.tmdbId];
    ...
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No Dark Comedy FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            <h2 className="text-xl sm:text-2xl font-light text-emerald-500 mb-6 sm:mb-8 flex items-center gap-2 sm:gap-3">
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base">
                Common questions about {movie?.Title} and its exploration of bleak humor, deep cynicism, and the absurdity of the human condition.
            </p>
...
export default DarkComedySEOFAQSection;