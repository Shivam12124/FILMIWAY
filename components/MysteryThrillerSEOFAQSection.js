// components/MysteryThrillerSEOFAQSection.js - DYNAMIC & SEO OPTIMIZED ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info } from 'lucide-react';
// 🔥 IMPORT THE DYNAMIC FAQ GENERATOR
import { getVisibleMovieFAQs } from '../utils/mysteryThrillerMovieData';
...
const MysteryThrillerSEOFAQSection = ({ movie }) => {
    ...
    return (
        ...
        >
            {/* 🎨 Updated to Indigo to match the Mystery Thriller theme */}
            <h2 className="text-xl sm:text-2xl font-light text-indigo-300 mb-6 sm:mb-8 flex items-center gap-2 sm:gap-3">
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base leading-relaxed">
                Explore our Parents Guide and expert analysis for {title}. We provide accurate timestamps for sensitive scenes and map the psychological complexity of this mystery.
            </p>
            ...
            <div className="space-y-4 sm:space-y-6">
                {faqsFromData.map((faq, index) => (
                    <motion.div
                        key={index}
                        className="bg-gray-800/30 rounded-xl p-4 sm:p-6 border border-gray-700/50 hover:border-indigo-500/50 transition-all duration-300"
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ duration: 0.6, delay: index * 0.1 }}
                    >
                        {/* 🔥 The exact question (Static Trivia OR Dynamic Parents Guide) */}
                        <h3 className="text-base sm:text-lg font-medium text-indigo-200 mb-3 sm:mb-4 leading-relaxed">
                            {faq.question || faq.q}
                        </h3>
