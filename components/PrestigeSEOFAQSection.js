// components/PrestigeSEOFAQSection.js
import React from 'react';
import { motion } from 'framer-motion';
import { Info } from 'lucide-react';
import { COMPLETE_MOVIE_DATA as PRESTIGE_MOVIE_DATA, getVisibleMovieFAQs } from '../utils/prestigeMovieData';

const PrestigeSEOFAQSection = ({ movie }) => {
    const movieInfo = PRESTIGE_MOVIE_DATA[movie?.tmdbId];
    ...
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No Prestige FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base">
                Common questions about {movie?.Title} and its exploration of rivalry, obsession, and the dark side of illusion.
            </p>
            <div className="space-y-4 sm:space-y-6">
                {faqsFromData.map((faq, index) => (
                    ...
                    >
                        <h3 className="text-base sm:text-lg font-medium text-blue-200 mb-2 sm:mb-3">{faq.question}</h3>
...
export default PrestigeSEOFAQSection;
