// components/ParamountFamilySEOFAQSection.js - PARAMOUNT+ FAMILY THEME (YELLOW/BLUE) ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info, Sparkles } from 'lucide-react';
import { PARAMOUNT_FAMILY_MOVIE_FAQS } from '../utils/paramountFamilyMovieData';

const ParamountFamilySEOFAQSection = ({ movie }) => {
    // 🔥 Get FAQs from PARAMOUNT_FAMILY_MOVIE_FAQS data
    const faqsFromData = movie?.Title && PARAMOUNT_FAMILY_MOVIE_FAQS?.[movie.Title] 
        ? PARAMOUNT_FAMILY_MOVIE_FAQS[movie.Title] 
        : [];

    // 🔥 Safety check - return null if no FAQs
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No Paramount+ Family FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base">
                Discover more about the fun, themes, and production of {movie.Title}.
            </p>
            <div className="space-y-4 sm:space-y-6">
                {faqsFromData.map((faq, index) => (
                    <motion.div 
                        key={index}
                        className="bg-gray-800/30 rounded-xl p-4 sm:p-6 border border-gray-700/50 hover:border-yellow-500/30 transition-all duration-300"
...
export default ParamountFamilySEOFAQSection;