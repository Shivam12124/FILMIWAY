// components/PeacockSciFiSEOFAQSection.js - COSMIC SCI-FI THEME (CYAN/BLUE) ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info, Sparkles } from 'lucide-react';
import { PEACOCK_SCIFI_MOVIE_FAQS } from '../utils/peacockSciFiMovieData';

const PeacockSciFiSEOFAQSection = ({ movie }) => {
    // 🔥 Get FAQs from PEACOCK_SCIFI_MOVIE_FAQS data
    const faqsFromData = movie?.Title && PEACOCK_SCIFI_MOVIE_FAQS?.[movie.Title] 
        ? PEACOCK_SCIFI_MOVIE_FAQS[movie.Title] 
        : [];

    // 🔥 Safety check - return null if no FAQs
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No Peacock Sci-Fi FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base">
                Deep dive into the themes, scientific accuracy, and production of {movie.Title} on Peacock.
            </p>
            <div className="space-y-4 sm:space-y-6">
                {faqsFromData.map((faq, index) => (
                    <motion.div 
                        key={index}
                        className="bg-gray-800/30 rounded-xl p-4 sm:p-6 border border-gray-700/50 hover:border-cyan-400/30 transition-all duration-300"
