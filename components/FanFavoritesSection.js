        ...
        <section className="mb-12 mt-8 animate-fade-in-up">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                {batch.map((slug, idx) => {
                    ...
                    return (
                        <Link key={idx} href={`/movie/${slug}/skip-timestamps`} onClick={handleFanFavoriteClick} className="group relative rounded-xl overflow-hidden aspect-[2/3] border border-white/5 hover:border-orange-500/50 transition-all duration-300 shadow-lg">
                            <Image src={`https://image.tmdb.org/t/p/w342${movieData.poster}`} alt={movieData.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 25vw, 150px" quality={40} />
