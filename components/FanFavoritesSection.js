        ...
        <section className="mb-12 mt-8 animate-fade-in-up">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                {similarMovies.map((movieData, idx) => {
                    ...
                    return (
                        <Link key={idx} href={`/movie/${movieData.slug}/skip-timestamps`} aria-label={`View parents guide and skip timestamps for ${movieData.title}`} onClick={handleFanFavoriteClick} className="group relative rounded-xl overflow-hidden aspect-[2/3] border border-white/5 hover:border-orange-500/50 transition-all duration-300 shadow-lg">
                            <Image
                                src={movieData.poster ? `https://image.tmdb.org/t/p/w185${movieData.poster}` : '/placeholder-poster.jpg'}
                                alt=""
                                fill
