...
const FanFavoritesSection = ({ currentMovieSlug, similarMovies }) => {
    ...
    return (
        <section className="mb-10 sm:mb-12 mt-6 sm:mt-8 animate-fade-in-up">
            <div className="flex items-center gap-2 sm:gap-2.5 mb-4 sm:mb-5">
                <Flame className="text-orange-500 w-5 h-5 sm:w-6 sm:h-6" />
                <h2 className="text-lg sm:text-xl lg:text-2xl font-medium sm:font-bold text-gray-200 sm:text-white">Similar Movie Guides</h2>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 lg:gap-5">
                {similarMovies.map((movieData, idx) => {
                    ...
                    return (
                        <Link key={idx} href={`/movie/${movieData.slug}/skip-timestamps`} aria-label={`View parents guide and skip timestamps for ${movieData.title}`} onClick={handleFanFavoriteClick} className="group relative rounded-xl sm:rounded-2xl overflow-hidden aspect-[2/3] border border-white/5 hover:border-orange-500/50 transition-all duration-300 shadow-lg">
                            <Image
                            ...
                            <div className="absolute bottom-0 left-0 w-full p-3 sm:p-4 transform translate-y-1 sm:translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                                <p className="text-white text-xs sm:text-sm lg:text-base font-medium sm:font-bold line-clamp-2 leading-tight drop-shadow-md">{movieData.title}</p>
                            </div>
                        </Link>
                    );
                })}
