// components/HuluComedySEOFAQSection.js - MATCHING COMEDY DESIGN ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info } from 'lucide-react';
import { HULU_COMEDY_MOVIE_FAQS } from '../utils/huluComedyMovieData';

const HuluComedySEOFAQSection = ({ movie }) => {
    // 🔥 Get FAQs from HULU_COMEDY_MOVIE_FAQS data
    const faqsFromData = movie?.Title && HULU_COMEDY_MOVIE_FAQS?.[movie.Title] 
        ? HULU_COMEDY_MOVIE_FAQS[movie.Title] 
        : [];

    // 🔥 Safety check - return null if no FAQs
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No Hulu Comedy FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base">
                Common questions about {movie.Title} and its place in comedy history.
            </p>
