// components/PeacockThrillerSEOFAQSection.js - MATCHING PEACOCK THRILLER THEME (Red/Crimson) ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info, Zap } from 'lucide-react'; // Added Zap for high energy thriller vibe
import { PEACOCK_THRILLER_MOVIE_FAQS } from '../utils/peacockThrillerMovieData';

const PeacockThrillerSEOFAQSection = ({ movie }) => {
    // 🔥 Get FAQs from PEACOCK_THRILLER_MOVIE_FAQS data
    const faqsFromData = movie?.Title && PEACOCK_THRILLER_MOVIE_FAQS?.[movie.Title] 
        ? PEACOCK_THRILLER_MOVIE_FAQS[movie.Title] 
        : [];

    // 🔥 Safety check - return null if no FAQs
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No Peacock Thriller FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base">
                Common questions about {movie.Title}, its twists, and its psychological impact.
            </p>
...
export default PeacockThrillerSEOFAQSection;