// components/EnemiesToLoversSEOFAQSection.js - MATCHING ROMANCE THEME ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Heart } from 'lucide-react'; // 💖 Swapped to Heart for the Romance theme

// 🔥 IMPORT THE NEW DYNAMIC FAQ GENERATOR
import { COMPLETE_MOVIE_DATA as ENEMIES_TO_LOVERS_MOVIE_DATA, getVisibleMovieFAQs } from '../utils/enemiesToLoversMovieData';

const EnemiesToLoversSEOFAQSection = ({ movie }) => {
    const movieInfo = ENEMIES_TO_LOVERS_MOVIE_DATA[movie?.tmdbId];
    ...
    if (!faqsFromData || faqsFromData.length === 0) {
        console.log('⚠️ No Enemies to Lovers FAQs found for:', movie?.Title);
        return null;
    }
    ...
    return (
        ...
        >
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base">
                Common questions about {movie?.Title} and its exploration of the enemies to lovers trope, romantic chemistry, and character dynamics.
            </p>
...
export default EnemiesToLoversSEOFAQSection;