// components/PeacockDramaSEOFAQSection.js - DYNAMIC & SEO OPTIMIZED ✅
import React from 'react';
import { motion } from 'framer-motion';
import { Info } from 'lucide-react';
// 🔥 IMPORT THE DYNAMIC FAQ GENERATOR
import { getVisibleMovieFAQs } from '../utils/peacockDramaMovieData';
...
const PeacockDramaSEOFAQSection = ({ movie }) => {
    ...
    return (
        ...
        >
            {/* 🎨 Cinematic Slate/Blue Theme for the Drama Collection */}
            <h2 className="text-xl sm:text-2xl font-light text-blue-400 mb-6 sm:mb-8 flex items-center gap-2 sm:gap-3">
                <Info size={20} className="sm:w-6 sm:h-6 text-blue-500" />
            ...
            <p className="text-gray-300 mb-4 sm:mb-6 text-sm sm:text-base leading-relaxed">
                Explore our **Parents Guide** and expert analysis for {title}. We provide accurate timestamps for sensitive scenes to skip and map the profound emotional resonance of this Peacock drama.
            </p>
            ...
            <div className="space-y-4 sm:space-y-6">
                {faqsFromData.map((faq, index) => (
                    <motion.div 
                        key={index}
                        className="bg-gray-800/30 rounded-xl p-4 sm:p-6 border border-gray-700/50 hover:border-blue-500/30 transition-colors duration-300"
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
                        transition={{ duration: 0.6, delay: index * 0.1 }}
                    >
                        {/* 🔥 The exact question (Static Trivia OR Dynamic Parents Guide) */}
                        <h3 className="text-base sm:text-lg font-medium text-blue-200 mb-2 sm:mb-3 leading-relaxed">
                            {faq.question || faq.q}
                        </h3>
