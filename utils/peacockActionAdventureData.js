...
// 🔥 1. THE KEYWORD BRIDGE (SEO-Optimized for Action/Adventure Content)
export const getSensitiveContentTypes = (tmdbId) => {
    ...
    sensitiveData.scenes.forEach(scene => {
        ...
        // Mapped to specific SEO terminology for mature/action content
...
export const generateCleanMovieSchema = (movie, tmdbData, currentMovieYear, collectionSlug, unused, movieInfo) => {
    ...
    const movieSchema = {
        ...
        '@type': 'Movie',
        'name': movie.Title,
        'description': movieInfo?.synopsis || `${movie.Title} (${currentMovieYear}) - High octane action and adventure movie on Peacock.`,
        'genre': movie.genre,
    ...
    const staticFaqs = PEACOCK_ACTION_ADVENTURE_FAQS[movie.Title] || [];
    ...
    // Inject Intensity Graph Timestamps into Schema (Mapped to Adrenaline Score)
    if (intensityScenes.length > 0) {
        ...
        schemaFaqs.unshift({
            ...
            'acceptedAnswer': { 
                '@type': 'Answer', 
                'text': `According to the Filmiway Adrenaline Score, ${movie.Title} peaks at the following moments:<br><br><ul>${schemaIntensityList}</ul>` 
            }
        });
    }
...
export const getVisibleMovieFAQs = (movieTitle, tmdbId, currentRuntime = "Official") => {
    const staticFaqs = PEACOCK_ACTION_ADVENTURE_FAQS[movieTitle] ? [...PEACOCK_ACTION_ADVENTURE_FAQS[movieTitle]] : [];
    ...
    if (intensityScenes.length > 0) {
        ...
        staticFaqs.unshift({
            ...
            answer: `According to the Filmiway Adrenaline Score, ${movieTitle} peaks at the following moments:\n\n${uiIntensityList}`
        });
    }
