...
function SearchResultCard({ item }) {
  return (
    <li
      tabIndex={0}
      role="link"
      aria-label={`Go to details for ${item.title || item.name}`}
      onClick={() => window.location.href = `/${item.media_type}/${item.id}`}
      className="cursor-pointer rounded-lg bg-[#111] shadow-md hover:shadow-yellow-500 transition-shadow duration-300 p-2 outline-none focus:ring-4 focus:ring-yellow-400"
    >
      ...
      <div className="px-4 pb-2">
        <h3 className="text-2xl font-extrabold text-white truncate mb-1">{item.title || item.name}</h3>
        <p className="uppercase text-yellow-500 font-semibold tracking-widest mb-1">
          {item.media_type === 'movie' ? 'MOVIE' : item.media_type === 'tv' ? 'TV SHOW' : 'PERSON'}
        </p>
        <p className="text-yellow-400 text-sm">
          {(item.release_date || item.first_air_date)?.slice(0, 4) ?? 'Unknown'} {item.vote_average ? `· ⭐ ${item.vote_average.toFixed(1)}` : ''}
        </p>
      </div>
    </li>
  );
...
export default function Results({ results, activeTab }) {
  ...
  if (!filtered.length) {
    return <p className="text-yellow-600 text-xl py-20 text-center">No results found.</p>;
  }
