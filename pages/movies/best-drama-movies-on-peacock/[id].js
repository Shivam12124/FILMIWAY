// pages/collection/best-drama-movies-on-peacock/[id].js
// PEACOCK DRAMA MOVIES COLLECTION
// VISUALS: Emotional & Cinematic Theme (Slate/Blue Accents)
...
// ✅ UPDATED MOVIE YEARS FOR PEACOCK DRAMA
const MOVIE_YEARS = {
...
// ✅ UPDATED MOVIE INSIGHTS FOR PEACOCK DRAMA
const MOVIE_DATA_BY_TITLE = {
    'Requiem for a Dream': { connection: 'A devastating visual masterpiece about the destructive power of addiction and the loss of dreams.' },
    'Sophie\'s Choice': { connection: 'Meryl Streep delivers the greatest performance in cinema history in this heartbreaking tale of survival and guilt.' },
    'The Fabelmans': { connection: 'Steven Spielberg\'s deeply personal memoir about family, forgiveness, and the magic of movies.' },
    'A Thousand and One': { connection: 'A raw, powerful portrait of a mother fighting against a broken system to keep her family together in Harlem.' },
    'Dìdi': { connection: 'A hilarious and painfully relatable coming-of-age story capturing the chaos of early adolescence in the digital age.' },
    'She Said': { connection: 'A gripping, meticulous procedural about the journalists who broke the silence and sparked a global movement.' },
    '50/50': { connection: 'A rare film that finds genuine humor and warmth in the face of a life-threatening illness. Perfectly balanced.' },
    'On Golden Pond': { connection: 'A timeless, tender exploration of aging, family reconciliation, and the enduring power of love.' },
    'Mrs. Harris Goes to Paris': { connection: 'A charming, uplifting fairy tale about following your dreams, no matter your age or background.' },
    'The Photograph': { connection: 'A lush, romantic drama exploring how past traumas shape our ability to love in the present.' }
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
    ...
    @media (max-width: 767px) {
        ...
        .mobile-psych-card { background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #3b82f6; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
        .mobile-psych-row { display: flex; align-items: flex-start; gap: 7px; }
        .mobile-psych-icon { min-width: 24px; min-height: 24px; color: #60a5fa; margin-top: 2px; }
        .mobile-psych-title { font-size: 15px; font-weight: bold; color: #60a5fa; margin-bottom: 1px; line-height: 1.12; }
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-drama-movies-on-peacock'; };
    return (
...
const PeacockDramaBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-drama-movies-on-peacock" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Drama Movies on Peacock</Link>
...
const PeacockDramaMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-drama-movies-on-peacock');
        sessionStorage.setItem('fromCollectionName', 'Best Drama Movies on Peacock');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <meta property="og:description" content="A powerful drama streaming on Peacock." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A powerful drama streaming on Peacock." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <PeacockDramaBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing the correct Peacock Drama context prop */}
                        <MovieDetailsSection movie={movie} fromPeacockDramaCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        {/* ✅ Updated Slug */}
                        <InternalCollectionsSection currentSlug="best-drama-movies-on-peacock" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default PeacockDramaMoviePage;