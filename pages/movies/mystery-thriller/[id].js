// pages/movies/mystery-thriller/[id].js
...
// Import mystery thriller movie data
...
const MysteryBackButton = () => {
  const handleBackClick = () => {
    if (typeof window !== 'undefined') {
      window.location.href = '/collection/best-mystery-thriller-movies';
    }
  };
...
const MysteryThrillerMoviePage = ({ movie }) => {
      ...
      <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
        <OptimizedBanner movie={movie} movieData={mergedMovieData} trailer={trailer} isMobile={isMobile} />
          ...
          <motion.div id="watch" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2, duration: 0.8 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
            <MovieDetailsSection movie={{ ...movie, Title: movie.title }} fromMysteryThrillerCollection={true} />
          </motion.div>
        </div>
        ...
        >
          ...
          <div className="flex justify-center items-center gap-8 sm:gap-12 mb-8">
            ...
            <div className="text-yellow-400/60 text-xs tracking-[0.3em] uppercase font-medium">
              Filmiway · Mystery Thriller Collection
            </div>
...
export async function getStaticPaths() {
  const paths = MYSTERY_THRILLER_MOVIES.map((movie) => ({
...
export async function getStaticProps({ params }) {
  const movie = MYSTERY_THRILLER_MOVIES.find((m) => m.imdbID === params.id);
