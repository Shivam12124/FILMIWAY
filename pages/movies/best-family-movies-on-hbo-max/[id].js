// pages/movies/best-family-movies-on-hbo-max/[id].js - HBO MAX FAMILY MOVIES
// VISUALS: Magical/Whimsical Theme (Yellow/Amber Accents)
// SCHEMA: Maximalist (Hidden Fun Factor, Wholesome Score, and FAQs for Bots)
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Sparkles, Smile, Theater } from 'lucide-react';
...
import { 
  ...
  COMPLETE_MOVIE_DATA,
  SENSITIVE_TIMELINES,
  HBO_MAX_FAMILY_MOVIE_FAQS 
} from '../../../utils/hboMaxFamilyMovieData';

const COLORS = {
  accent: '#FACC15', accentLight: '#FEF08A',bgPrimary: '#000000ff', bgCard: 'rgba(11, 11, 11, 0.8)',// Yellow/Slate Base
...
const MOVIE_DATA_BY_TITLE = {
  'The Wizard of Oz': { connection: 'The ultimate family classic. A timeless journey about courage, heart, and finding your way home.' },
  'Harry Potter and the Prisoner of Azkaban': { connection: 'The best-directed Harry Potter film. It balances magic with genuine character growth and thrilling mystery.' },
  'Flow': { connection: 'A dialogue-free masterpiece. Stunning visuals and pure storytelling make it a unique experience for all ages.' },
  'A Christmas Story': { connection: 'Nostalgic, hilarious, and perfectly captures the chaotic joy of childhood holidays.' },
  'Puss in Boots: The Last Wish': { connection: 'Visually spectacular and surprisingly deep. It tackles big themes with humor and breathtaking animation.' },
  'Kiki\'s Delivery Service': { connection: 'A gentle, beautiful story about finding your independence and believing in yourself. Pure Ghibli magic.' },
  'Batman: Mask of the Phantasm': { connection: 'Often called the best Batman movie ever. A tragic, animated noir that respects both kids and adults.' },
  'When Marnie Was There': { connection: 'An emotional ghost story about friendship and healing. Beautifully animated and deeply moving.' },
  'Kung Fu Panda': { connection: 'Perfect blend of action, comedy, and philosophy. Jack Black brings heart to a visually stunning martial arts epic.' },
  'The Lego Batman Movie': { connection: 'A hilarious, meta-commentary on Batman that is packed with jokes for kids and adults alike.' }
};
...
const getFamilyInsight = (title) => {
  ...
  return data?.connection || 'A wonderful family movie perfect for movie night.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  // ✅ UPDATED: Using 'funFactor' directly
  const funFactor = richData?.funFactor || 90; 
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-psych-card">
            <div className="mobile-psych-row"><Sparkles className="mobile-psych-icon" /><div><div className="mobile-psych-title">Fun Factor</div></div></div>
            <div className="mobile-psych-desc"><strong>{funFactor}/100</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-family-movies-on-hbo-max'; };
    return (
...
const HboFamilyBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-family-movies-on-hbo-max" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Family Movies on HBO Max</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = HBO_MAX_FAMILY_MOVIE_FAQS[movie.Title] || [];
  ...
  const fullDescription = `
    ...
    Ranking: #${movie.rank || 'N/A'} in HBO Max Family Movies.
  ...
  const movieSchema = {
    ...
    "keywords": "Family Movies HBO Max, Kids Movies, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/collection/best-family-movies-on-hbo-max/${movie.imdbID}`, 
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const HboFamilyMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-family-movies-on-hbo-max');
        sessionStorage.setItem('fromCollectionName', 'Best Family Movies on HBO Max');
    }
  }, []);
    ...
    const cleanSEOTitle = `${movie.Title} (${currentMovieYear}) - Best Family Movies on HBO Max | Filmiway`;
    const cleanSEODesc = `${movie.Title} (${currentMovieYear}) - A magical family movie streaming on HBO Max. Ranked by wholesomeness and fun factor.`;
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/collection/best-family-movies-on-hbo-max/${movie.imdbID}`} />
                ...
                <meta property="og:description" content="A great family movie on HBO Max." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A great family movie on HBO Max." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <HboFamilyBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-family-movies-on-hbo-max" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default HboFamilyMoviePage;