// pages/movies/best-a24-movies/[id].js - H1 SEO FIX + HYDRATION FIX ✅
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Theater, Heart } from 'lucide-react';
...
// ✅ CORRECTED IMPORT: Pulling schema generator from its dedicated A24 file
import {
...
const MOVIE_YEARS = {
  'Aftersun': '2022', 'The Zone of Interest': '2023', 'The Florida Project': '2017', 'The Lighthouse': '2019',
  'The Iron Claw': '2023', 'Lady Bird': '2017', 'Hereditary': '2018', 'Midsommar': '2019',
  'Close': '2022', 'The Whale': '2022'
};

const MOVIE_DATA_BY_TITLE = {
  'Aftersun': { connection: 'A devastatingly subtle exploration of memory, depression, and a daughter trying to understand her father.' },
  'The Zone of Interest': { connection: 'A chilling, formally rigorous masterpiece that exposes the terrifying banality of evil right next door.' },
  'The Florida Project': { connection: 'A vibrant, heartbreaking look at childhood innocence surviving in the shadows of grinding poverty.' },
  'The Lighthouse': { connection: 'A claustrophobic, mythic descent into madness driven by isolation, guilt, and toxic masculinity.' },
  'The Iron Claw': { connection: 'An emotionally crushing, beautifully acted tragedy about brotherhood and the devastating weight of family expectations.' },
  'Lady Bird': { connection: 'A perfect, universally resonant coming-of-age story about identity, home, and the complex love between mothers and daughters.' },
  'Hereditary': { connection: 'A landmark modern horror film that weaponizes family grief and generational trauma into a deeply disturbing nightmare.' },
  'Midsommar': { connection: 'A sun-drenched, visceral fairy tale about toxic relationships, profound grief, and terrifying catharsis.' },
  'Close': { connection: 'A profoundly moving, delicate examination of childhood friendship abruptly destroyed by the pressures of toxic masculinity.' },
  'The Whale': { connection: 'A contained, emotionally massive character study about grief, self-destruction, and the desperate search for redemption.' }
};
...
const getEmotionalInsight = (title) => {
  ...
  return data?.connection || 'A profound exploration of the human condition, pushing the boundaries of modern cinema.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const emotionalResonance = richData?.emotionalResonance || 85;
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-psych-card">
            <div className="mobile-psych-row"><Heart className="mobile-psych-icon" /><div><div className="mobile-psych-title">Emotional Resonance</div></div></div>
            <div className="mobile-psych-desc"><strong>{emotionalResonance}</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(234, 179, 8, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(234, 179, 8, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This Hits Deep</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Emotional Resonance: {emotionalResonance}/100</p></div>
                </div>
...
const AuthorCreditSection = () => (
  <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
      <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of modern cinema masterpieces</p></div></div>
...
const A24MovieBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/best-a24-movies" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>10 Best A24 Movies</Link>
...
const A24MoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-a24-movies');
        sessionStorage.setItem('fromCollectionName', '10 Best A24 Movies');
    }
  }, []);
  ...
  const collectionSlug = 'best-a24-movies';
  ...
  return (
      <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
          ...
          <div className="relative z-10 pt-20 sm:pt-24 lg:pt-28">
              ...
              <A24MovieBreadcrumb movie={movie} />
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <motion.div id="watch" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
                      <MovieDetailsSection movie={movie} fromA24Collection={true} />
                  </motion.div>
                  
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="best-a24-movies" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
