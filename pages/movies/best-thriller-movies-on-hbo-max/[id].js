...
const COLORS = {
  accent: '#EF4444', accentLight: '#F87171', bgPrimary: '#0f172a', bgCard: 'rgba(15, 23, 42, 0.8)', // Red/Slate
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-thriller-movies-on-hbo-max'; };
    return (
...
const HboMaxThrillerBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-thriller-movies-on-hbo-max" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Thriller Movies on HBO Max</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = HBO_THRILLER_MOVIE_FAQS[movie.Title] || [];
  ...
  const intensityStats = `
    ...
    - Psychological Pressure: ${data?.psychologicalPressure || 0}/100
  `;
  ...
  const fullDescription = `
    ...
    Ranking: #${movie.rank || 'N/A'} in HBO Max Thrillers.
  ...
  const movieSchema = {
    ...
    "keywords": "Thriller Movies HBO Max, Best Suspense Films, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/movies/best-thriller-movies-on-hbo-max/${movie.imdbID}`, 
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const HboMaxThrillerMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-thriller-movies-on-hbo-max');
        sessionStorage.setItem('fromCollectionName', 'Best Thriller Movies on HBO Max');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/movies/best-thriller-movies-on-hbo-max/${movie.imdbID}`} />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <HboMaxThrillerBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing HBO Max Thriller Context */}
                        <MovieDetailsSection movie={movie} fromHboMaxThrillerCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-thriller-movies-on-hbo-max" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default HboMaxThrillerMoviePage;