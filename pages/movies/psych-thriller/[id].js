// pages/movies/psych-thriller/[id].js
...
// Use local synopsis insights for psychological thrillers
const getPsychThrillerInsight = (title) => {
  switch (title) {
    case 'The Silence of the Lambs':
      return 'A definitive, high-stakes psychological chess match with every nuance of mentor and predator blurred.';
    case 'Fight Club':
      return 'A mind-bending exploration of identity, consumerism, and primal aggression spiraling into chaos.';
    case 'Memento':
      return 'Memory loss forces an intricate reverse narrative as the protagonist hunts a murderer.';
    case 'Se7en':
      return 'Dark, grim detective thriller centered on serial killings inspired by the seven deadly sins.';
    case 'Shutter Island':
      return 'Psychological thriller featuring a reality-bending plunge into paranoia and suspicious asylum secrets.';
    case 'Black Swan':
      return 'Terrifying psychological breakdown amidst the pressures and chaos of ballet perfection.';
    case 'The Machinist':
      return 'A haunting, dizzying journey through paranoia and fragmented reality.';
    case 'Oldboy':
      return 'A brutal revenge quest unfolding a horrifying and premeditated plot.';
    case 'Taxi Driver':
      return 'An isolated veteran’s violent descent into obsession in grim New York streets.';
    case 'The Shining':
      return 'Supernatural terror driving a writer into murderous madness in an isolated hotel.';
    default:
      return 'An intense psychological thriller full of complex characters and suspense.';
  }
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile }) => {
  ...
  const bannerImage =
    movieData?.backdrop_path
      ...
      : movieData?.backdrop || movie?.backdrop;
  ...
  const thrillerInsight = getPsychThrillerInsight(movie?.title);
...
const BackButton = () => {
  const handleBackClick = () => {
    if(typeof window !== 'undefined'){
      window.location.href = '/collection/best-psychological-thriller-movies';
    }
  };
...
const PsychThrillerMoviePage = ({ movie }) => {
  ...
  useEffect(() => {
    if(typeof window !== 'undefined') {
      sessionStorage.setItem('fromPsychThrillerCollection', 'true');
  ...
  return (
    <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
      <Head>
  <title>{movie.title} ({movie.year}) - Best Psychological Thriller | Filmiway</title>
  <meta name="description" content={`${movie.title} (${movie.year}) - ${movie.synopsis?.substring(0, 150) || 'Psychological thriller film'}...`} />
  ...
  {/* Open Graph */}
  <meta property="og:title" content={`${movie.title} (${movie.year}) - Psychological Thriller`} />
  <meta property="og:description" content={movie.synopsis?.substring(0, 120) || 'A psychological thriller film'} />
  <meta property="og:type" content="video.movie" />
  <meta property="og:url" content={`https://filmiway.com/movies/psych-thriller/${movie.imdbID}`} />
  ...
  <meta name="twitter:description" content={movie.synopsis?.substring(0, 120) || 'A psychological thriller'} />
      ...
      <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
        ...
        >
          ...
          <div className="flex justify-center items-center gap-8 sm:gap-12 mb-8">
            ...
            <div className="text-purple-500/60 text-xs tracking-[0.3em] uppercase font-medium">
              Filmiway · Psychological Thriller Collection
            </div>
...
export async function getStaticPaths() {
  const paths = PSYCH_THRILLER_MOVIES.map((movie) => ({
...
export async function getStaticProps({ params }) {
  const movie = PSYCH_THRILLER_MOVIES.find((m) => m.imdbID === params.id);
