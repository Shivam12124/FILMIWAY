// pages/movies/psych-thriller/[id].js - H1 SEO FIX + HYDRATION FIX ✅
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Brain, AlertTriangle } from 'lucide-react'; 
...
const COLORS = {
  accent: '#7c3aed', // Violet/Purple for Psych Thriller
  accentLight: '#a78bfa', 
  bgPrimary: '#0B0B0C', 
...
const MOVIE_YEARS = {
  'The Silence of the Lambs': '1991', 'Fight Club': '1999', 'Memento': '2000', 'Se7en': '1995',
  'Shutter Island': '2010', 'Black Swan': '2010', 'The Machinist': '2004', 'Oldboy': '2003',
  'Taxi Driver': '1976', 'The Shining': '1980'
};

const MOVIE_DATA_BY_TITLE = {
  'The Silence of the Lambs': { connection: 'A definitive, high-stakes psychological chess match with every nuance of mentor and predator blurred.' },
  'Fight Club': { connection: 'A mind-bending exploration of identity, consumerism, and primal aggression spiraling into chaos.' },
  'Memento': { connection: 'Memory loss forces an intricate reverse narrative as the protagonist hunts a murderer through a fragmented reality.' },
  'Se7en': { connection: 'Dark, grim detective thriller centered on serial killings inspired by the seven deadly sins, pushing characters to their breaking points.' },
  'Shutter Island': { connection: 'Psychological thriller featuring a reality-bending plunge into paranoia and suspicious asylum secrets.' },
  'Black Swan': { connection: 'Terrifying psychological breakdown amidst the pressures and chaos of ballet perfection and identity fragmentation.' },
  'The Machinist': { connection: 'A haunting, dizzying journey through paranoia, insomnia, and guilt that physically manifests in the protagonist.' },
  'Oldboy': { connection: 'A brutal revenge quest unfolding a horrifying and premeditated plot that challenges the limits of human endurance.' },
  'Taxi Driver': { connection: 'An isolated veteran’s violent descent into obsession and vigilantism in the grim streets of 1970s New York.' },
  'The Shining': { connection: 'Supernatural terror driving a writer into murderous madness in an isolated hotel, blurring the line between ghost story and mental collapse.' }
};

const getTMDBImage = (path, size = 'w1280') =>
  path ? `https://image.tmdb.org/t/p/${size}${path}` : undefined;

const getPsychInsight = (title) => {
  const data = MOVIE_DATA_BY_TITLE[title];
  return data?.connection || 'An intense psychological thriller full of complex characters and mind-bending suspense.';
};

// ✅ OPTIMIZED BANNER (Hydration fix applied: Auto-countdown removed)
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const psychIntensity = richData?.sanityErosion || richData?.psychologicalIntensity || 90;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-psych-card { background: linear-gradient(135deg, #1e1b4b 0%, #0f172a 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #7c3aed; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-psych-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-psych-icon { min-width: 24px; min-height: 24px; color: #a78bfa; margin-top: 2px; }
    .mobile-psych-title { font-size: 15px; font-weight: bold; color: #a78bfa; margin-bottom: 1px; line-height: 1.12; }
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-psych-card">
            <div className="mobile-psych-row"><Brain className="mobile-psych-icon" /><div><div className="mobile-psych-title">Sanity Erosion</div></div></div>
        ...
        <div className="relative px-3 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 z-20" style={{ backgroundColor: COLORS.bgPrimary }}>
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 items-start">
            <motion.div className="flex-shrink-0 relative w-24 sm:w-48 md:w-56 lg:w-80 mx-auto sm:mx-0" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.8 }}>
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(124, 58, 237, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(124, 58, 237, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This Gets Under Your Skin</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Sanity Erosion Level: {psychIntensity}/100</p></div>
                </div>
                ...
                <motion.div className="mt-4 sm:mt-6 h-0.5 sm:h-1 rounded-full" style={{ background: `linear-gradient(90deg, ${COLORS.accent}60, transparent)`, width: '40%' }} initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: 0.8, duration: 0.8 }} />
              </motion.div>
            </motion.div>
          </div>
        </div>
...
const SmartBackButton = () => {
  const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-psychological-thriller-movies'; };
  return (
...
const AuthorCreditSection = () => (
  <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.0, duration: 0.8 }}>
...
const PsychBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/best-psychological-thriller-movies" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Psychological Thrillers</Link>
...
const PsychThrillerMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-psychological-thriller-movies');
        sessionStorage.setItem('fromCollectionName', 'Best Psychological Thriller Movies');
    }
  }, []);
          ...
          <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
              ...
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="best-psychological-thriller-movies" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
...
export default PsychThrillerMoviePage;