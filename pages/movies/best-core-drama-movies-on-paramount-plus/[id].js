...
const COLORS = {
  accent: '#eab308',           // Gold
  accentLight: '#fde047',      // Light Gold
  bgPrimary: '#020617',        // Deep Indigo/Slate Black
  bgCard: 'rgba(15, 23, 42, 0.8)', // Translucent Slate
  textPrimary: '#FFFFFF', 
...
const MOVIE_DATA_BY_TITLE = {
  'There Will Be Blood': { connection: 'A visceral examination of the corrupting nature of ambition and the hollowness of absolute power.' },
  'The English Patient': { connection: 'A sweeping, poetic tragedy about the haunting persistence of memory and forbidden love.' },
  'Fences': { connection: 'A thunderous adaptation of August Wilson’s play exploring generational trauma and deferred dreams.' },
  'Dead Man Walking': { connection: 'A profound ethical inquiry into justice, mercy, and the heavy burden of forgiveness.' },
  'Selma': { connection: 'A stirring, humanizing portrait of leadership and the collective courage required to shift history.' },
  'Silence': { connection: 'A brutal and beautiful meditation on the silence of God and the endurance of faith.' },
  'One Life': { connection: 'A deeply moving testament to the impact of quiet, ordinary heroism in the face of evil.' },
  'I, Daniel Blake': { connection: 'A heartbreakingly realistic critique of bureaucratic cruelty and the resilience of human dignity.' },
  'The Curious Case of Benjamin Button': { connection: 'A visually wondrous fable about the inevitability of time and the preciousness of every moment.' },
  'Forrest Gump': { connection: 'An iconic journey through history viewed through the lens of innocence and unconditional love.' }
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-core-drama-movies-on-paramount-plus'; };
    return (
...
const ParamountDramaBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-core-drama-movies-on-paramount-plus" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Core Drama Movies on Paramount+</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = PARAMOUNT_DRAMA_MOVIE_FAQS[movie.Title] || [];
  ...
  const movieSchema = {
    ...
    "keywords": "Best Drama Movies Paramount+, Emotional Films, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/collection/best-core-drama-movies-on-paramount-plus/${movie.imdbID}`, 
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const ParamountDramaMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-core-drama-movies-on-paramount-plus');
        sessionStorage.setItem('fromCollectionName', 'Best Core Drama Movies on Paramount+');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/collection/best-core-drama-movies-on-paramount-plus/${movie.imdbID}`} />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-core-drama-movies-on-paramount-plus" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default ParamountDramaMoviePage;