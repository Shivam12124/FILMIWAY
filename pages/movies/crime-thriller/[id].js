// pages/movies/crime-thriller/[id].js - H1 SEO FIX + HYDRATION FIX ✅
...
import { ChevronLeft, ChevronRight, Play, X, User, Twitter, Hash, Send, Shield, Film } from 'lucide-react';
...
const COLORS = {
  accent: '#64748b', // Slate Blue/Grey for Crime
  accentLight: '#94a3b8', 
  bgPrimary: '#0B0B0C', 
  bgCard: 'rgba(30, 41, 59, 0.3)', // Dark Slate background
  textPrimary: '#FFFFFF', 
...
const MOVIE_DATA_BY_TITLE = {
  'Se7en': { connection: 'Se7en is the ultimate descent into darkness. Two detectives hunt a killer using the seven deadly sins as his blueprint.' },
  'The Silence of the Lambs': { connection: 'The definitive psychological crime thriller. FBI trainee Clarice Starling must gain the trust of imprisoned cannibal Dr. Hannibal Lecter.' },
  'Zodiac': { connection: 'Zodiac is obsession incarnate. Based on the true story of the Zodiac Killer, showing how an unsolved case consumes lives.' },
  'Prisoners': { connection: 'Prisoners explores how far a desperate father will go when his daughter vanishes. A morally complex, tension-soaked thriller.' },
  'Memories of Murder': { connection: 'Bong Joon-ho\'s haunting masterpiece about South Korea\'s first serial murders. Detectives struggle with limited resources and corrupt systems.' },
  'The Girl with the Dragon Tattoo': { connection: 'Fuses mystery with revenge. Journalist Mikael Blomkvist and hacker Lisbeth Salander uncover twisted family secrets.' },
  'Gone Girl': { connection: 'A marriage from hell. When Amy Dunne disappears, her husband becomes the prime suspect in a sharp tale of media manipulation.' },
  'Nightcrawler': { connection: 'A descent into sociopathy. Lou Bloom crosses every ethical line for the perfect shot - a dark mirror of media sensationalism.' },
  'No Country for Old Men': { connection: 'Relentless fate personified. A hunter finds drug money and becomes prey to Anton Chigurh, one of cinema\'s most terrifying villains.' },
  'Mystic River': { connection: 'Tragedy layered on trauma. When a daughter is murdered, three childhood friends are pulled back into a shared past of violence.' }
};
...
const getCrimeInsight = (title) => {
  ...
  return data?.connection || 'An extraordinary exploration of crime, justice, and the darkness within humanity.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  // METRIC CHANGE: Crime Intensity
  const crimeIntensity = richData?.crimeIntensity || 90;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-crime-card { background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #64748b; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-crime-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-crime-icon { min-width: 24px; min-height: 24px; color: #94a3b8; margin-top: 2px; }
    .mobile-crime-title { font-size: 15px; font-weight: bold; color: #94a3b8; margin-bottom: 1px; line-height: 1.12; }
    .mobile-crime-desc { font-size: 12.3px; color: #ededed; line-height: 1.36; margin-top: 2px; }
  }`;
          ...
          {showTrailer && trailer ? (
            ...
            <motion.div key="image" className="absolute inset-0 rounded-none sm:rounded-3xl overflow-hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
              <div className="relative w-full h-full">
                ...
                <div className="absolute inset-0 z-10" style={{ background: `linear-gradient(to bottom, transparent 0%, transparent 60%, ${COLORS.bgPrimary}90 85%, ${COLORS.bgPrimary} 100%)` }} />
              </div>
              {trailer && (
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 items-start">
            <motion.div className="flex-shrink-0 relative w-24 sm:w-48 md:w-56 lg:w-80 mx-auto sm:mx-0" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.8 }}>
              <div className="relative" style={{ aspectRatio: '2/3' }}>
                {posterImage ? <Image src={posterImage} alt={`${movie?.Title} poster`} fill sizes="(max-width: 640px) 96px, (max-width: 768px) 192px, (max-width: 1024px) 224px, 320px" quality={85} className="object-cover rounded-lg sm:rounded-xl shadow-2xl" /> : <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: COLORS.bgCard, borderRadius: '12px' }}><Shield style={{ color: COLORS.textMuted }} /></div>}
              </div>
            </motion.div>
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(100, 116, 139, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(100, 116, 139, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  <motion.div className="p-2 sm:p-2.5 rounded-lg sm:rounded-xl flex-shrink-0" style={{ background: `linear-gradient(135deg, ${COLORS.accent}20, ${COLORS.accent}10)`, border: `1px solid ${COLORS.accent}40` }} whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }}><Shield className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" style={{ color: COLORS.accent }} /></motion.div>
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This Crime Thriller Hits Hard</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Crime Intensity: {crimeIntensity}/100</p></div>
                </div>
...
const AuthorCreditSection = () => (
    <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
            <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of crime cinema</p></div></div>
...
const CrimeBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            ...
            <ChevronRight size={14} className="flex-shrink-0" style={{ color: COLORS.textDisabled, transform: 'rotate(180deg)' }} /><span className="font-medium truncate" style={{ color: `${COLORS.accent}B3` }}>{movie.Title}</span>
        </div>
    </motion.nav>
);

// ✅ JSON-LD SCHEMA GENERATOR - CRIME EDITION
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  const data = COMPLETE_MOVIE_DATA[movie.tmdbId];
  const sensitiveData = SENSITIVE_TIMELINES[movie.tmdbId];
  const faqs = CRIME_THRILLER_FAQS[movie.Title] || [];
  ...
  // 2. METRICS (Updated for Crime)
  const intensityStats = `
    [FILMIWAY METRICS]
    - Crime Intensity: ${data?.crimeIntensity || 0}/100
  ...
  const fullDescription = `
    ${data?.synopsis || movie.description || "A gripping crime thriller."}
    ...
    Ranking: #${movie.rank || 'N/A'} in Crime Thrillers.
  ...
  const movieSchema = {
    ...
    "genre": data?.dna ? Object.keys(data.dna) : ["Crime", "Thriller"],
    "keywords": "Crime Thriller, Mystery, Detective, Suspense, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/movies/crime-thriller/${movie.imdbID}`,
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const CrimeThrillerMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-crime-thriller-movies');
        sessionStorage.setItem('fromCollectionName', 'Best Crime Thriller Movies');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                {/* ✅ HIDDEN H1 FOR SEO */}
                <h1 className="sr-only">{`${movie.Title} (${currentMovieYear}) - Best Crime Thriller Movies`}</h1>
                ...
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-crime-thriller-movies" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
