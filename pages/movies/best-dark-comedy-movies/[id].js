// pages/movies/best-dark-comedy-movies/[id].js - H1 SEO FIX + HYDRATION FIX ✅
// VISUALS: Minimalist (Banner + Details Only) with Cynical/Dark Aesthetic
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Smile, Theater } from 'lucide-react';
...
// ✅ CORRECTED IMPORT: Pulling schema generator from its dedicated Dark Comedy file
import {
  COMPLETE_MOVIE_DATABASE, 
  COMPLETE_MOVIE_DATA,
  SENSITIVE_TIMELINES,
  DARK_COMEDY_MOVIE_FAQS,
  generateCleanMovieSchema
} from '../../../utils/darkComedyMovieData';

const COLORS = {
  accent: '#10B981', accentLight: '#34D399', bgPrimary: '#09090b', bgCard: 'rgba(15, 15, 20, 0.8)',
  textPrimary: '#FFFFFF', textSecondary: '#E5E7EB', textMuted: '#9CA3AF', textDisabled: '#6B7280',
  borderAccent: 'rgba(16, 185, 129, 0.25)', borderLight: 'rgba(63, 63, 70, 0.5)',
};

const MOVIE_YEARS = {
  'Dr. Strangelove': '1964', 'Network': '1976', 'The Banshees of Inisherin': '2022', 'In Bruges': '2008',
  'Withnail and I': '1987', 'The Death of Stalin': '2017', 'What We Do in the Shadows': '2014', 'Adaptation.': '2002',
  'Election': '1999', 'Burn After Reading': '2008'
};

const MOVIE_DATA_BY_TITLE = {
  'Dr. Strangelove': { connection: 'The undisputed apex of political satire, turning the terrifying prospect of global nuclear annihilation into the darkest, sharpest joke ever told.' },
  'Network': { connection: 'A prophetic, blistering scream against corporate media and public apathy that only gets more cynical and accurate with time.' },
  'The Banshees of Inisherin': { connection: 'A bleak, profoundly human tragicomedy about male loneliness, stubborn pride, and the escalating absurdity of a petty feud.' },
  'In Bruges': { connection: 'A hilarious, guilt-ridden existential crisis wrapped in rapid-fire profanity, set against the purgatorial backdrop of a medieval fairytale town.' },
  'Withnail and I': { connection: 'A booze-soaked, desperately hilarious eulogy for the 1960s counter-culture, exploring failed ambition and toxic friendship.' },
  'The Death of Stalin': { connection: 'A frantic, darkly hilarious look at the absolute absurdity of totalitarian power vacuums and the terrified men scrambling to survive them.' },
  'What We Do in the Shadows': { connection: 'A brilliant mockumentary that forces the grand, gothic terror of immortal vampires into the awkward, mundane reality of modern flat-sharing.' },
  'Adaptation.': { connection: 'A dizzying, self-loathing meta-comedy about the agonizing creative process and the desperate desire to be someone else.' },
  'Election': { connection: 'A razor-sharp satire of ruthless ambition and petty spite, proving that high school politics are just as vicious as the real world.' },
  'Burn After Reading': { connection: 'A masterclass in nihilistic comedy where absolutely everyone is an idiot, and zero lessons are learned from a trail of pointless chaos.' }
};

const getTMDBImage = (path, size = 'w1280') =>
  path ? `https://image.tmdb.org/t/p/${size}${path}` : undefined;

const getDarkComedyInsight = (title) => {
  const data = MOVIE_DATA_BY_TITLE[title];
  return data?.connection || 'A masterpiece of dark humor, exploring the absolute absurdity and bleakness of the human condition.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const insight = getDarkComedyInsight(movie?.Title);
  const cynicismLevel = richData?.cynicismLevel || 85;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-psych-card { background: linear-gradient(135deg, #022c22 0%, #0f0f15 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid ${COLORS.accent}; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-psych-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-psych-icon { min-width: 24px; min-height: 24px; color: ${COLORS.accentLight}; margin-top: 2px; }
    .mobile-psych-title { font-size: 15px; font-weight: bold; color: ${COLORS.accentLight}; margin-bottom: 1px; line-height: 1.12; }
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-psych-card">
            <div className="mobile-psych-row"><Smile className="mobile-psych-icon" /><div><div className="mobile-psych-title">Cynicism Level</div></div></div>
            <div className="mobile-psych-desc"><strong>{cynicismLevel}</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(16, 185, 129, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(16, 185, 129, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This Hits Deep</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Cynicism Level: {cynicismLevel}/100</p></div>
                </div>
...
const AuthorCreditSection = () => (
  <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
      <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of dark comedy and satire</p></div></div>
...
const DarkComedyMovieBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/best-dark-comedy-movies" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>10 Best Dark Comedy Movies</Link>
...
const DarkComedyMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-dark-comedy-movies');
        sessionStorage.setItem('fromCollectionName', '10 Best Dark Comedy Movies');
    }
  }, []);
  ...
  const collectionSlug = 'best-dark-comedy-movies';
  ...
  return (
      <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
          ...
          <div className="relative z-10 pt-20 sm:pt-24 lg:pt-28">
              ...
              <DarkComedyMovieBreadcrumb movie={movie} />
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <motion.div id="watch" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
                      <MovieDetailsSection movie={movie} fromDarkComedyCollection={true} />
                  </motion.div>
                  
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="best-dark-comedy-movies" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
...
export default DarkComedyMoviePage;