// pages/collection/best-romance-movies-on-peacock/[id].js
// PEACOCK ROMANCE MOVIES COLLECTION
...
// ✅ IMPORT PEACOCK ROMANCE DATA
import { 
    COMPLETE_MOVIE_DATABASE, 
    COMPLETE_MOVIE_DATA,
    SENSITIVE_TIMELINES,
    PEACOCK_ROMANCE_MOVIE_FAQS 
} from '../../../utils/peacockRomanceMovieData';

const COLORS = {
    accent: '#EC4899',         // Pink 500
    accentLight: '#FBCFE8',    // Pink 200
    bgPrimary: '#000000ff',    // Black
    bgCard: 'rgba(11, 11, 11, 0.9)', 
    textPrimary: '#FFFFFF', 
    textSecondary: '#FCE7F3',  // Pink 100
    textMuted: '#DB2777',      // Pink 600
    textDisabled: '#9D174D',   // Pink 800
    borderAccent: 'rgba(236, 72, 153, 0.4)', 
    borderLight: 'rgba(131, 24, 67, 0.5)', // Pink 900
};

// ✅ UPDATED MOVIE YEARS FOR PEACOCK ROMANCE
const MOVIE_YEARS = {
    'About Time': '2013',
...
// ✅ UPDATED MOVIE INSIGHTS FOR PEACOCK ROMANCE
const MOVIE_DATA_BY_TITLE = {
    'About Time': { connection: 'A quiet masterpiece proving love isn\'t just about finding a partner, but cherishing every ordinary day.' },
    'Pride & Prejudice': { connection: 'The gold standard of period romance. The hand-flex scene alone has spawned a thousand TikTok edits.' },
    'Notting Hill': { connection: 'The ultimate "star meets ordinary guy" fantasy. Julia Roberts and Hugh Grant define 90s rom-com chemistry.' },
    'Ghost': { connection: 'Supernatural eternal love. It combines thriller elements with deep romance to show connection transcends death.' },
    'Shakespeare in Love': { connection: 'A witty, passionate reimagining of history that suggests the greatest plays are born from the greatest heartbreaks.' },
    'One Day': { connection: 'A devastating look at timing. It captures the joy and agony of two people who are perfect for each other, just never at the same time.' },
    'Challengers': { connection: 'Romance as a contact sport. A high-energy, sweaty, and intense love triangle that plays out on and off the tennis court.' },
    'The Age of Adaline': { connection: 'Timeless elegance. A unique fantasy romance about the loneliness of immortality and the courage to finally grow old with someone.' },
    'How to Lose a Guy in 10 Days': { connection: 'The peak of the 2000s rom-com era. Kate Hudson and Matthew McConaughey have unmatched, chaotic chemistry.' },
    'The Favourite': { connection: 'A twisted, dark, and hilarious lesbian love triangle involving a Queen. It’s romance stripped of all sentimentality and replaced with power.' }
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-romance-movies-on-peacock'; };
    return (
...
const AuthorCreditSection = () => (
    <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
            <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of Romance cinema</p></div></div>
...
// ✅ RENAMED TO PEACOCK ROMANCE
const PeacockRomanceBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-romance-movies-on-peacock" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Romance Movies on Peacock</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
    ...
    const faqs = PEACOCK_ROMANCE_MOVIE_FAQS[movie.Title] || [];
    ...
    if (data?.scenes && data.scenes.length > 0) {
        ...
        peakStats = `[PEAK EMOTION] Maximum Intensity (${peakScene.intensity}/100) hits at minute ${peakScene.time}: "${peakScene.label}".`;
    }

    const intensityStats = `
        [FILMIWAY METRICS]
        - Emotional Intensity: ${data?.emotionalIntensity || 0}/100
        - Chemistry Score: ${data?.chemistryScore || 0}/100
    `;
    ...
    const contentWarnings = sensitiveData?.scenes 
        ...
        : 'Standard cinematic romance themes.';
    ...
    const fullDescription = `
        ...
        Ranking: #${movie.rank || 'N/A'} in Romance Collections.
    ...
    const movieSchema = {
        ...
        "keywords": "Romance Movies Peacock, Top Romantic Films, " + (data?.themes ? data.themes.join(", ") : ""),
        "url": `https://filmiway.com/movies/best-romance-movies-on-peacock/${movie.imdbID}`, 
...
// ✅ RENAMED TO PEACOCK ROMANCE
const PeacockRomanceMoviePage = ({ movie, tmdbData: movieData }) => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            sessionStorage.setItem('fromCollection', 'best-romance-movies-on-peacock');
            sessionStorage.setItem('fromCollectionName', 'Best Romance Movies on Peacock');
        }
    }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/movies/best-romance-movies-on-peacock/${movie.imdbID}`} />
                ...
                <meta property="og:description" content="A deeply emotional romance movie on Peacock." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A deeply emotional romance movie on Peacock." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <PeacockRomanceBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* Note: Ensure MovieDetailsSection can handle the 'romance' context if it has specific logic */}
                         
                    </motion.div><MovieDetailsSection movie={movie} fromPeacockRomanceCollection={true} />
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        {/* ✅ INTERNAL COLLECTION SLUG UPDATED */}
                        <InternalCollectionsSection currentSlug="best-romance-movies-on-peacock" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default PeacockRomanceMoviePage;