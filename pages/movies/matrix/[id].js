// pages/movies/matrix/[id].js - H1 SEO FIX + HYDRATION FIX ✅
...
const COLORS = {
  ...
  textMuted: '#9CA3AF', 
  textDisabled: '#6B7280',
  borderAccent: 'rgba(0, 255, 0, 0.25)', 
  borderLight: 'rgba(107, 114, 128, 0.3)',
};

const MOVIE_YEARS = {
  'Dark City': '1998', 'Inception': '2010', 'The Thirteenth Floor': '1999', 'eXistenZ': '1999',
  'Source Code': '2011', 'Minority Report': '2002', 'Upgrade': '2018', 'Total Recall': '1990',
  'Blade Runner 2049': '2017', 'Edge of Tomorrow': '2014'
};

const MOVIE_DATA_BY_TITLE = {
  'Dark City': { connection: 'Dark City predates The Matrix with its own noir-futuristic vision of a city where memories are altered and reality shifts nightly.' },
  'Inception': { connection: 'Inception uses dream technology as its construct-within-construct premise. Nolan\'s layered narrative mirrors The Matrix\'s exploration of reality vs illusion.' },
  'The Thirteenth Floor': { connection: 'A direct spiritual sibling to The Matrix, exploring multiple layers of simulated realities and questioning the nature of existence itself.' },
  'eXistenZ': { connection: 'Cronenberg\'s trippy masterpiece presents virtual reality through biological interface technology. Reality bleeds with illusion.' },
  'Source Code': { connection: 'A time-loop thriller where a soldier can inhabit the body of another person. Like The Matrix, it explores consciousness transfer.' },
  'Minority Report': { connection: 'Examines a high-tech future where precrime technology blurs free will and destiny. Navigates a world of advanced AI and neural interfaces.' },
  'Upgrade': { connection: 'A gritty indie sci-fi about a paralyzed man implanted with an AI. Explores themes of consciousness merging with technology.' },
  'Total Recall': { connection: 'Predates The Matrix by asking: what if your memories are false? Schwarzenegger\'s journey through constructed worlds explores identity.' },
  'Blade Runner 2049': { connection: 'A visually stunning cyberpunk epic exploring what it means to be human. Replicants questioning consciousness mirrors The Matrix.' },
  'Edge of Tomorrow': { connection: 'Tom Cruise relives the same day in an alien war. The time-loop mechanic mirrors The Matrix\'s exploration of escaping predetermined patterns.' }
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  @media (max-width: 767px) {
    ...
    .mobile-matrix-card { background: linear-gradient(135deg, #0a1a0a 0%, #002200 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #00FF00; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-matrix-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-matrix-icon { min-width: 24px; min-height: 24px; color: #00FF41; margin-top: 2px; }
    .mobile-matrix-title { font-size: 15px; font-weight: bold; color: #00FF41; margin-bottom: 1px; line-height: 1.12; }
    .mobile-matrix-desc { font-size: 12.3px; color: #ededed; line-height: 1.36; margin-top: 2px; }
  }`;
    ...
    <motion.div className="relative w-full overflow-hidden mb-6 sm:mb-8 mx-0 sm:mx-4 lg:mx-6 rounded-none sm:rounded-3xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          <div className="mobile-hero-poster">{posterImage ? <Image src={posterImage} alt={`${movie?.Title} poster`} width={320} height={480} className="w-full h-auto" priority /> : <div style={{ background: COLORS.bgCard, width: '100%', height: '150px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Cpu style={{ color: COLORS.textMuted }} /></div>}</div>
          <div className="mobile-matrix-card">
            <div className="mobile-matrix-row"><Cpu className="mobile-matrix-icon" /><div><div className="mobile-matrix-title">Sci-Fi Complexity</div></div></div>
            <div className="mobile-matrix-desc"><strong>{complexityScore}</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 items-start">
            <motion.div className="flex-shrink-0 relative w-24 sm:w-48 md:w-56 lg:w-80 mx-auto sm:mx-0" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.8 }}>
              <div className="relative" style={{ aspectRatio: '2/3' }}>
                {posterImage ? <Image src={posterImage} alt={`${movie?.Title} poster`} fill sizes="(max-width: 640px) 96px, (max-width: 768px) 192px, (max-width: 1024px) 224px, 320px" quality={85} className="object-cover rounded-lg sm:rounded-xl shadow-2xl" /> : <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: COLORS.bgCard, borderRadius: '12px' }}><Cpu style={{ color: COLORS.textMuted }} /></div>}
              </div>
            </motion.div>
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(0, 255, 0, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(0, 255, 0, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  <motion.div className="p-2 sm:p-2.5 rounded-lg sm:rounded-xl flex-shrink-0" style={{ background: `linear-gradient(135deg, ${COLORS.accent}20, ${COLORS.accent}10)`, border: `1px solid ${COLORS.accent}40` }} whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }}><Cpu className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" style={{ color: COLORS.accent }} /></motion.div>
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This is Like The Matrix</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Sci-Fi Complexity: {complexityScore}/100</p></div>
                </div>
...
// ✅ JSON-LD SCHEMA GENERATOR - MATRIX EDITION
...
const MatrixMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'movies-like-the-matrix');
        sessionStorage.setItem('fromCollectionName', 'Movies Like The Matrix');
    }
  }, []);
