// pages/collection/best-sci-fi-movies-on-paramount-plus/[id].js
// PARAMOUNT+ SCI-FI MOVIES COLLECTION
...
// ✅ IMPORT PARAMOUNT SCI-FI DATA
import { 
    COMPLETE_MOVIE_DATABASE, 
    COMPLETE_MOVIE_DATA,
    SENSITIVE_TIMELINES,
    PARAMOUNT_SCIFI_MOVIE_FAQS 
...
// ✅ MOVIE YEARS FOR PARAMOUNT SCI-FI
const MOVIE_YEARS = {
    'Interstellar': '2014',
    'Arrival': '2016',
    'The Martian': '2015',
...
const MOVIE_DATA_BY_TITLE = {
    'Interstellar': { connection: 'The undisputed king of modern sci-fi. Nolan’s mix of black hole physics, time dilation, and father-daughter emotion makes it a Tier 1 masterpiece.' },
    'Arrival': { connection: 'A thinking person’s alien movie. Directed by Denis Villeneuve, the twist at the end changes the entire genre. Essential viewing.' },
    'The Martian': { connection: 'The ultimate survival movie. It’s Into the Wild but on Mars, and with a happy ending. Smart, funny, and incredibly gripping engineering sci-fi.' },
    'Minority Report': { connection: 'Spielberg and Cruise at their peak. It predicts the future of technology perfectly while telling a dark, mind-bending detective story.' },
    'Annihilation': { connection: 'Cosmic horror meets psychedelic beauty. If you like Donnie Darko or Black Swan, this disturbing, visual masterpiece is for you.' },
    'A Quiet Place': { connection: 'A masterclass in suspense. It uses sound—or the lack of it—as a weapon. Short, punchy, and incredibly effective tension.' },
    'Star Trek II: The Wrath of Khan': { connection: 'The best Star Trek movie ever made. It defines the space opera genre with its iconic submarine-style battle in the nebula.' },
    '10 Cloverfield Lane': { connection: 'Claustrophobic suspense at its finest. 90% of the movie is just three people in a bunker questioning reality. Is the monster real, or is the man crazy?' },
    'A.I. Artificial Intelligence': { connection: 'A collaboration between Kubrick and Spielberg. It is a deeply philosophical, melancholy look at robotics that is becoming more relevant every day.' },
    'War of the Worlds': { connection: 'Captures the fear of sci-fi perfectly. The Tripod sounds and the river scene are legendary moments of invasion panic.' }
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-sci-fi-movies-on-paramount-plus'; };
    return (
...
const ParamountSciFiBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-sci-fi-movies-on-paramount-plus" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Sci-Fi Movies on Paramount+</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
    ...
    const faqs = PARAMOUNT_SCIFI_MOVIE_FAQS[movie.Title] || [];
    ...
    const movieSchema = {
        ...
        "keywords": "Sci-Fi Movies Paramount Plus, Visionary Science Fiction, " + (data?.themes ? data.themes.join(", ") : ""),
        "url": `https://filmiway.com/movies/best-sci-fi-movies-on-paramount-plus/${movie.imdbID}`, 
...
const ParamountSciFiMoviePage = ({ movie, tmdbData: movieData }) => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            sessionStorage.setItem('fromCollection', 'best-sci-fi-movies-on-paramount-plus');
            sessionStorage.setItem('fromCollectionName', 'Best Sci-Fi Movies on Paramount+');
        }
    }, []);
    ...
    const cleanSEOTitle = `${movie.Title} (${currentMovieYear}) - Best Sci-Fi Movies on Paramount+ | Filmiway`;
    const cleanSEODesc = `${movie.Title} (${currentMovieYear}) - A visionary sci-fi masterpiece streaming on Paramount+. Ranked by complexity and visual scale.`;
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/movies/best-sci-fi-movies-on-paramount-plus/${movie.imdbID}`} />
                <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0" />
                <meta name="robots" content="index, follow" />
                <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(movieSchema) }} />
                {faqSchema && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }} />}
                <meta property="og:title" content={cleanSEOTitle} />
                <meta property="og:description" content="A visionary science fiction movie on Paramount+." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A visionary science fiction movie on Paramount+." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <ParamountSciFiBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        <MovieDetailsSection movie={movie} fromParamountSciFiCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-sci-fi-movies-on-paramount-plus" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default ParamountSciFiMoviePage;