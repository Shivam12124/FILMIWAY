// pages/best-romance-movies-on-hulu/[id].js - HULU ROMANCE MOVIES
// VISUALS: Romantic/Warm Theme (Pink/Rose Accents)
// SCHEMA: Maximalist (Hidden Emotional Resonance, Chemistry, and FAQs for Bots)
...
import { 
  ...
  COMPLETE_MOVIE_DATA,
  SENSITIVE_TIMELINES,
  HULU_ROMANCE_MOVIE_FAQS 
} from '../../../utils/huluRomanceMovieData';

const COLORS = {
  accent: '#EC4899', accentLight: '#FBCFE8', bgPrimary: '#0f172a', bgCard: 'rgba(23, 23, 30, 0.8)', // Pink/Rose for Romance
...
const MOVIE_DATA_BY_TITLE = {
  ...
  'Palm Springs': { connection: 'A clever deconstruction of commitment. It uses the time-loop trope to explore what it really means to choose someone every day.' },
  'Rye Lane': { connection: 'A visual love letter to modern dating and South London. It feels fresh, colorful, and genuinely optimistic.' },
  'Crazy Heart': { connection: 'A rugged, booze-soaked redemption arc. It shows how love can inspire art even when it cannot save the artist.' },
  'Good Luck to You, Leo Grande': { connection: 'An intimate chamber piece about body positivity and the radical act of allowing oneself to feel pleasure.' },
  '13 Going on 30': { connection: 'The ultimate comfort watch. A reminder that the best kind of love is often the one that has been there all along.' }
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const emotionalScore = richData?.psychologicalIntensity || 85; // Mapping 'psychologicalIntensity' to Emotional Resonance
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-psych-card">
            <div className="mobile-psych-row"><Heart className="mobile-psych-icon" /><div><div className="mobile-psych-title">Emotional Score</div></div></div>
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(236, 72, 153, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(236, 72, 153, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why It Resonates</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Emotional Resonance: {emotionalScore}/100</p></div>
                </div>
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-romance-movies-on-hulu'; };
    return (
...
const HuluRomanceBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/best-romance-movies-on-hulu" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Romance Movies on Hulu</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const intensityStats = `
    [FILMIWAY METRICS]
    - Emotional Resonance: ${data?.psychologicalIntensity || 0}/100
    - Romantic Tension: ${data?.destructiveObsession || 0}/100
    - Chemistry Score: ${data?.visceralImpact || 0}/100
  `;
  ...
  const movieSchema = {
    ...
    "url": `https://filmiway.com/best-romance-movies-on-hulu/${movie.imdbID}`,
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const HuluRomanceMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-romance-movies-on-hulu');
        sessionStorage.setItem('fromCollectionName', 'Best Romance Movies on Hulu');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <meta property="og:description" content="A top romance movie on Hulu." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A top romance movie on Hulu." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <HuluRomanceBreadcrumb movie={movie} />
...
>
                        ...
                        <MovieDetailsSection movie={movie} fromHuluRomanceCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-romance-movies-on-hulu" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default HuluRomanceMoviePage;