// pages/movies/netflix/[id].js - H1 SEO FIX + HYDRATION FIX + DRAMA SCHEMA ✅
...
// ✅ SCHEMA GENERATOR (DRAMA EDITION) - CUSTOM METRICS
const generateMovieSchema = (movie, movieData) => {
  ...
  const intensityStats = `
    ...
    - Character Depth: ${characterDepth}/100
    - Narrative Power: ${narrativePower}/100
  `;
  
  const fullDescription = `
    ${movie.synopsis || "An award-winning drama film."}
    ...
    Ranking: #${movie.rank || 'N/A'} in Best Netflix Dramas.
  ...
  return {
    ...
    "image": movieData?.poster_path ? getTMDBImage(movieData.poster_path, 'w500') : undefined,
    "director": {
      "@type": "Person",
    ...
    "review": {
      ...
      "reviewRating": {
        "@type": "Rating",
        "ratingValue": movie.imdbRating || 8.0,
        "bestRating": "10",
        "worstRating": "1"
      }
    },
    "genre": ["Drama"],
    "keywords": "Drama Movie, Emotional Cinema, Character Study, " + (movie.genre ? movie.genre : ""),
    "url": `https://filmiway.com/movies/netflix/${movie.imdbID}`,
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile }) => {
  ...
  @media (max-width: 767px) {
    ...
    .mobile-drama-card { background: #19181c; border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #E50914; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-drama-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-drama-icon { min-width: 24px; min-height: 24px; color: #E50914; margin-top: 2px; }
    .mobile-drama-title { font-size: 15px; font-weight: bold; color: #e50914; margin-bottom: 1px; line-height: 1.12; }
    .mobile-drama-desc { font-size: 12.3px; color: #ededed; line-height: 1.36; margin-top: 2px; }
  }`;
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-drama-card">
            <div className="mobile-drama-row"><Award className="mobile-drama-icon" /><div><div className="mobile-drama-title">Essential Drama</div></div></div>
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, ${COLORS.bgCard} 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(0, 0, 0, 0.4)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This is Essential Drama</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Expert Analysis</p></div>
                </div>
                <div className="relative pl-4 sm:pl-6 border-l-2" style={{ borderColor: `${COLORS.accent}40` }}>
                  ...
                  <p className="text-xs sm:text-sm lg:text-base xl:text-lg leading-relaxed font-normal break-words" style={{ color: COLORS.textSecondary, lineHeight: '1.8' }}>{dramaInsight}</p>
                </div>
...
const DramaBackButton = () => {
  const handleBackClick = () => { if (typeof window !== "undefined") window.location.href = "/collection/best-drama-movies-on-netflix"; };
  return (
    ...
    <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
      ...
      <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
        ...
        <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
          ...
          {/* Footer Section */}
          <motion.footer className="relative z-20 bg-gradient-to-t from-gray-900 to-black mt-16 pt-12 border-t border-gray-800/30 text-center w-full" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.8 }}>
            <p className="text-gray-500 text-sm sm:text-base font-light tracking-wide mb-8 px-4">Curated cinema for discerning viewers — Expert analysis and community insights</p>
            <div className="flex justify-center items-center gap-8 sm:gap-12 mb-8">
              ...
              <div className="text-red-500/60 text-xs tracking-[0.3em] uppercase font-medium">Filmiway · Netflix Drama Collection</div>
...
export async function getStaticPaths() {
  const paths = DRAMA_MOVIES.map((movie) => ({ params: { id: movie.imdbID } }));
