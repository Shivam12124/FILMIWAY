...
import { Play, X, Sparkles, Film, ChevronLeft } from 'lucide-react';
...
const getSciFiInsight = (title) => {
  switch (title) {
    case '2001: A Space Odyssey':
      return 'A transcendent masterpiece of science fiction that explores human evolution and cosmic destiny. Stanley Kubrick\'s visionary work defies conventional narrative, challenging viewers with profound philosophical questions about existence and technology.';
    case 'Blade Runner':
      return 'A neo-noir sci-fi thriller that questions what it means to be human. Ridley Scott\'s atmospheric vision of a rain-soaked future remains a timeless exploration of identity, consciousness, and mortality in a technological age.';
    case 'The Matrix':
      return 'A revolutionary action-packed sci-fi film that fundamentally changed cinema. The Matrix presents a stunning vision of reality as simulation, combined with groundbreaking visual effects and profound philosophical themes about awakening.';
    case 'Alien':
      return 'A masterpiece of sci-fi horror that redefined the genre. Ridley Scott\'s claustrophobic masterpiece merges science fiction with primal fear, creating an unforgettable tension-filled experience in the depths of space.';
    case 'Interstellar':
      return 'An epic emotional odyssey across space and time that explores humanity\'s survival. Christopher Nolan\'s ambitious film combines stunning visuals with a deeply moving story about love transcending dimensions and human resilience.';
    case 'Terminator 2: Judgment Day':
      return 'A groundbreaking action sci-fi film that set new standards for visual effects and practical stunts. James Cameron\'s sequel is a relentless thrill ride exploring themes of destiny, redemption, and humanity\'s fight against extinction.';
    case 'Inception':
      return 'A visually spectacular and intellectually complex thriller about dreams within dreams. Christopher Nolan crafts an intricate, layered narrative that challenges perception and questions the nature of reality itself.';
    case 'Arrival':
      return 'A thoughtful, emotional sci-fi drama about communication and time. Denis Villeneuve presents a cerebral exploration of language, alien contact, and what it means to understand something profoundly different from ourselves.';
    case 'Star Wars: A New Hope':
      return 'An epic space opera that revolutionized cinema and captured imaginations worldwide. George Lucas created an entire galaxy of adventure, heroism, and mythology that became a cultural phenomenon.';
    case 'Blade Runner 2049':
      return 'A visually breathtaking and philosophically profound sequel that expands the original\'s themes. Denis Villeneuve\'s dystopian masterpiece explores identity, memory, and what it truly means to be alive in a future where replicants exist.';
    default:
      return 'A captivating exploration of humanity\'s future and imagination. This film showcases the boundless possibilities of science fiction filmmaking.';
  }
};
  ...
  <div className="mobile-hero-row">
    ...
    <div className="mobile-scifi-card">
      <div className="mobile-scifi-row">
        <Sparkles className="mobile-scifi-icon" />
        <div>
          <div className="mobile-scifi-title">Sci-Fi Index</div>
        </div>
      </div>
      <div className="mobile-scifi-desc">
        <strong>{movie?.sciFiComplexity || 92}</strong> - {insight}
      </div>
    </div>
  </div>
) : (
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 items-start">
            ...
            >
              <motion.div
                ...
                style={{
                  background: `linear-gradient(135deg, ${COLORS.bgCard} 0%, rgba(15, 43, 52, 0.5) 100%)`,
...
const SciFiMoviePage = ({ movie }) => {
  ...
  return (
    <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
      <Head>
        <title>{movie.title} ({movie.year}) - Top 10 Sci-Fi | Filmiway</title>
        <meta name="description" content={`${movie.title} (${movie.year}) - ${movie.synopsis?.substring(0, 150) || 'Sci-Fi film'}...`} />
        <link rel="canonical" href={`https://filmiway.com/movies/sci-fi/${movie.imdbID}`} />
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0" />
        <meta name="robots" content="index, follow" />
        <meta property="og:url" content={`https://filmiway.com/movies/sci-fi/${movie.imdbID}`} />
      ...
      <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
        ...
        <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
          <motion.div id="watch" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2, duration: 0.8 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
            <MovieDetailsSection movie={{ ...movie, Title: movie.title }} fromSciFiCollection={true} />
          </motion.div>
        </div>
...
export async function getStaticPaths() {
  const paths = SCI_FI_MOVIES.map((movie) => ({
