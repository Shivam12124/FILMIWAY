// pages/movies/movies-like-fight-club/[id].js - H1 SEO FIX + HYDRATION FIX ✅
// VISUALS: Minimalist (Banner + Details Only) with Gritty/Red Aesthetic
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Flame, Eye } from 'lucide-react';
...
// ✅ CORRECTED IMPORT: Pulling schema generator from its dedicated Fight Club file
import {
  COMPLETE_MOVIE_DATABASE, 
  COMPLETE_MOVIE_DATA,
  SENSITIVE_TIMELINES,
  FIGHT_CLUB_MOVIE_FAQS,
  generateCleanMovieSchema
} from '../../../utils/fightClubMovieData';

const COLORS = {
  accent: '#DC2626', accentLight: '#F87171', bgPrimary: '#09090b', bgCard: 'rgba(15, 15, 20, 0.8)',
  textPrimary: '#FFFFFF', textSecondary: '#E5E7EB', textMuted: '#9CA3AF', textDisabled: '#6B7280',
  borderAccent: 'rgba(220, 38, 38, 0.25)', borderLight: 'rgba(63, 63, 70, 0.5)',
};

const MOVIE_YEARS = {
  'Filth': '2013', 'Choke': '2008', 'The Double': '2013', 'The Art of Self-Defense': '2019',
  'Falling Down': '1993', 'American History X': '1998', 'Brazil': '1985', 'Office Space': '1999',
  'V for Vendetta': '2005', 'Being John Malkovich': '1999'
};

const MOVIE_DATA_BY_TITLE = {
  'Filth': { connection: 'A hyper-aggressive plunge into total psychological collapse, driven by a corrupt, unreliable narrator losing his grip on reality.' },
  'Choke': { connection: 'Written by the author of Fight Club, sharing the exact same DNA of cynical humor, alienated men, and destructive anti-capitalist themes.' },
  'The Double': { connection: 'A surreal, claustrophobic exploration of identity fracture and the creation of a darker, more confident alter-ego to survive.' },
  'The Art of Self-Defense': { connection: 'A pitch-black satire exploring toxic masculinity, physical violence, and the terrifying allure of cult-like indoctrination.' },
  'Falling Down': { connection: 'The explosive, terrifying result of an ordinary, repressed man snapping against the indignities of modern consumer society.' },
  'American History X': { connection: 'A brutal examination of charismatic leadership, ideological radicalization, and the catastrophic cycle of violence.' },
  'Brazil': { connection: 'A surreal, Kafkaesque nightmare about rebelling against a soul-crushing, hyper-bureaucratic system that erases individuality.' },
  'Office Space': { connection: 'The comedic, white-collar equivalent of Fight Club\'s anti-corporate, anti-work ethos. Total rebellion through absolute apathy.' },
  'V for Vendetta': { connection: 'A highly stylized, ideological crusade of absolute anarchic destruction against a fascist, totalitarian regime.' },
  'Being John Malkovich': { connection: 'A bizarre, existential trip into the commodification of identity, highlighting the desperate urge to escape one\'s own skin.' }
};
...
const getPsychologicalInsight = (title) => {
  ...
  return data?.connection || 'A dark, anarchic exploration of identity, systemic rebellion, and psychological collapse.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const anarchyLevel = richData?.anarchyLevel || 85;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-psych-card { background: linear-gradient(135deg, #180909 0%, #0f0f15 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid ${COLORS.accent}; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-psych-card">
            <div className="mobile-psych-row"><Flame className="mobile-psych-icon" /><div><div className="mobile-psych-title">Anarchy Level</div></div></div>
            <div className="mobile-psych-desc"><strong>{anarchyLevel}</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(220, 38, 38, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(220, 38, 38, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This Hits Deep</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Anarchy Level: {anarchyLevel}/100</p></div>
                </div>
...
const FightClubMovieBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/movies-like-fight-club" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Movies Like Fight Club</Link>
...
const FightClubMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'movies-like-fight-club');
        sessionStorage.setItem('fromCollectionName', 'Movies Like Fight Club');
    }
  }, []);
  ...
  const collectionSlug = 'movies-like-fight-club';
  ...
  return (
      <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
          ...
          <div className="relative z-10 pt-20 sm:pt-24 lg:pt-28">
              ...
              <FightClubMovieBreadcrumb movie={movie} />
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <motion.div id="watch" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
                      <MovieDetailsSection movie={movie} fromFightClubCollection={true} />
                  </motion.div>
                  
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="movies-like-fight-club" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
...
export default FightClubMoviePage;