// pages/movies/parasite/[id].js - H1 SEO FIX + HYDRATION FIX ✅
...
const COLORS = {
  ...
  borderAccent: 'rgba(236, 72, 153, 0.25)', borderLight: 'rgba(107, 114, 128, 0.3)',
};
...
const MOVIE_DATA_BY_TITLE = {
  'Parasite': { connection: 'Parasite is the ultimate class warfare thriller. A poor family infiltrates a wealthy household, exposing how capitalism creates parasites on both ends of the spectrum.' },
  'The Handmaiden': { connection: 'The Handmaiden deconstructs colonial class hierarchies through seduction and deception, where wealth determines power—until women reclaim their agency.' },
  'Burning': { connection: 'Burning is class resentment as slow-burn psychological horror. The wealthy can destroy people and disappear, while the poor can only watch and spiral into obsession.' },
  'Snowpiercer': { connection: 'Snowpiercer is literally a class system on rails. Revolution doesn\'t bring freedom—it just reveals that those in power designed the system to be eternal.' },
  'Memories of Murder': { connection: 'Memories of Murder shows institutional failure as class injustice. Poor suspects are tortured into false confessions while the wealthy are protected.' },
  'Shoplifters': { connection: 'Shoplifters asks: what makes a family? The marginalized must steal to survive while society abandons them, criminalizing poverty while ignoring neglect.' },
  'The Platform': { connection: 'The Platform is pure allegory for capitalism\'s cruelty. Those above feast while those below starve. Humanity\'s selfishness ensures the system perpetuates.' },
  'Mother': { connection: 'Mother reveals how class prejudice corrupts justice. Poor defendants are assumed guilty while mothers obsessively protect their children against a rigged system.' },
  'The Killing of a Sacred Deer': { connection: 'The Killing of a Sacred Deer uses Greek tragedy to explore privilege. A surgeon\'s wealth cannot protect him from moral punishment; guilt is universal.' },
  'The Housemaid': { connection: 'The Housemaid exposes sexual exploitation as class exploitation. The wealthy sexually abuse workers with impunity, leaving revenge as the only justice.' },
  'The Talented Mr. Ripley': { connection: 'The Talented Mr. Ripley is class envy as existential crisis. Tom Ripley doesn\'t just want money—he wants the ease of being born into privilege.' }
};
...
const getClassWarfareInsight = (title) => {
  ...
  return data?.connection || 'A brilliant exploration of class, power, and human nature.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  // METRIC CHANGE: Class Warfare Index
  const classWarfareIndex = richData?.classWarfareIndex || 85;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-class-card { background: linear-gradient(135deg, #1a0f15 0%, #0f0f10 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #EC4899; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-class-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-class-icon { min-width: 24px; min-height: 24px; color: #F472B6; margin-top: 2px; }
    .mobile-class-title { font-size: 15px; font-weight: bold; color: #F472B6; margin-bottom: 1px; line-height: 1.12; }
    .mobile-class-desc { font-size: 12.3px; color: #ededed; line-height: 1.36; margin-top: 2px; }
  }`;
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-class-card">
            <div className="mobile-class-row"><Brain className="mobile-class-icon" /><div><div className="mobile-class-title">Class Index</div></div></div>
            <div className="mobile-class-desc"><strong>{classWarfareIndex}</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(236, 72, 153, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(236, 72, 153, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This Class War Film Matters</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Class Warfare Index: {classWarfareIndex}/100</p></div>
                </div>
...
// ✅ JSON-LD SCHEMA GENERATOR - PARASITE EDITION
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  const data = COMPLETE_MOVIE_DATA[movie.tmdbId];
  const sensitiveData = SENSITIVE_TIMELINES[movie.tmdbId];
  const faqs = PARASITE_MOVIE_FAQS[movie.Title] || [];
  ...
  // 2. METRICS (Metrics Changed for Parasite)
  const intensityStats = `
    [FILMIWAY METRICS]
    - Class Warfare Index: ${data?.classWarfareIndex || 0}/100
    - Thriller Elements: ${data?.thrillerElements || 0}/100
  `;
  ...
  const fullDescription = `
    ${data?.synopsis || movie.description || "A brilliant class warfare film."}
    ...
    Ranking: #${movie.rank || 'N/A'} in Class Warfare Cinema.
  ...
  const movieSchema = {
    ...
    "genre": data?.dna ? Object.keys(data.dna) : ["Thriller", "Drama"],
    "keywords": "Parasite, Class Warfare, Inequality, Bong Joon Ho, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/movies/parasite/${movie.imdbID}`,
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const ParasiteMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'movies-like-parasite');
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                {/* 🔥 HYDRATION BUG FIX */}
                <title>{`${movie.Title} (${currentMovieYear}) - Movies Like Parasite | Filmiway`}</title>
                <meta name="description" content={`${movie.Title} (${currentMovieYear}) - A brilliant class warfare film like Parasite. Analysis, ratings & where to stream.`} />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                {/* ✅ HIDDEN H1 FOR SEO */}
                <h1 className="sr-only">{`${movie.Title} (${currentMovieYear}) - Movies Like Parasite`}</h1>
