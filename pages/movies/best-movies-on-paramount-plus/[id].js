// pages/collection/best-movies-on-paramount-plus/[id].js - PARAMOUNT+ BEST MOVIES
...
// ✅ IMPORT PARAMOUNT BEST DATA
import { 
  COMPLETE_MOVIE_DATABASE, 
  COMPLETE_MOVIE_DATA,
  SENSITIVE_TIMELINES,
  PARAMOUNT_BEST_MOVIE_FAQS 
...
const MOVIE_DATA_BY_TITLE = {
  'Pulp Fiction': { connection: 'Quentin Tarantino\'s non-linear masterpiece. It redefined cinema with its sharp dialogue, eclectic soundtrack, and cool violence.' },
  'Forrest Gump': { connection: 'A journey through 20th-century history. Tom Hanks delivers a performance that captures the heart and soul of America.' },
  'Interstellar': { connection: 'A visual spectacle rooted in hard science. Christopher Nolan explores the power of love across the vastness of space and time.' },
  'Gladiator': { connection: 'The epic that revitalized the historical genre. Ridley Scott combines visceral arena combat with a touching story of honor.' },
  'Sunset Boulevard': { connection: 'The dark side of Hollywood fame. A noir classic that remains one of the most cynical and brilliant films about the industry.' },
  'The Wolf of Wall Street': { connection: 'A three-hour adrenaline shot of greed. Scorsese and DiCaprio turn white-collar crime into a hilarious, terrifying spectacle.' },
  'No Country for Old Men': { connection: 'A masterclass in tension. The Coen Brothers strip away music and comfort to create a stark, nihilistic thriller.' },
  'There Will Be Blood': { connection: 'A study of American capitalism and greed. Daniel Day-Lewis gives a performance of titanic, terrifying intensity.' },
  'Top Gun: Maverick': { connection: 'The ultimate blockbuster experience. It pushes the boundaries of practical effects and honors the legacy of the original.' },
  'Catch Me If You Can': { connection: 'A stylish, breezy caper. Spielberg captures the cool essence of the 60s in this true story of a brilliant teenage con artist.' }
};
...
const getMovieInsight = (title) => {
  ...
  return data?.connection || 'A masterpiece of cinema streaming on Paramount+.';
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-movies-on-paramount-plus'; };
    return (
...
const ParamountBestBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-movies-on-paramount-plus" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Movies on Paramount+</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = PARAMOUNT_BEST_MOVIE_FAQS[movie.Title] || [];
  ...
  const intensityStats = `
    ...
    - Cinematic Mastery: ${data?.cinematicMastery || 90}/100
  `;
  ...
  const fullDescription = `
    ${data?.synopsis || movie.description || "A top-rated movie streaming on Paramount+."}
    ...
    Ranking: #${movie.rank || 'N/A'} in Best Movies on Paramount+.
  ...
  const movieSchema = {
    ...
    "keywords": "Best Movies Paramount+, Top Rated Films, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/collection/best-movies-on-paramount-plus/${movie.imdbID}`,
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const ParamountBestMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-movies-on-paramount-plus');
        sessionStorage.setItem('fromCollectionName', 'Best Movies on Paramount+');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/collection/best-movies-on-paramount-plus/${movie.imdbID}`} />
                ...
                <meta property="og:description" content="One of the best movies streaming on Paramount+." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="One of the best movies streaming on Paramount+." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <ParamountBestBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing context for Paramount Best collection */}
                        <MovieDetailsSection movie={movie} fromParamountBestCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-movies-on-paramount-plus" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default ParamountBestMoviePage;