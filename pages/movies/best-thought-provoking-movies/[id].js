// pages/movies/best-thought-provoking-movies/[id].js - INDIGO PHILOSOPHY THEME ✅
// 100% SEO SAFE: Framed around Philosophical Depth, Emotional Resonance, and Human Texture
// SCHEMA: Maximalist (Hidden Philosophical Score and FAQs for Bots)
...
const COLORS = {
  accent: '#4F46E5', // Deep Indigo for Philosophy/Thought
  accentLight: '#818CF8', 
  bgPrimary: '#000000ff', 
  bgCard: 'rgba(11, 11, 11, 0.8)',
  textPrimary: '#FFFFFF', textSecondary: '#E5E7EB', textMuted: '#9CA3AF', textDisabled: '#6B7280',
  borderAccent: 'rgba(79, 70, 229, 0.25)', borderLight: 'rgba(55, 65, 81, 0.5)',
};
...
const getPhilosophicalInsight = (title) => {
  const data = MOVIE_DATA_BY_TITLE[title];
  return data?.connection || 'A profound cinematic experience that challenges your perception of the human condition.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const insight = getPhilosophicalInsight(movie?.Title);
  // ✅ CUSTOM METRIC: Philosophical Depth Score
  const philosophicalDepth = richData?.philosophicalDepth || 85;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-thought-card { background: linear-gradient(135deg, #0f1123 0%, #08080a 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #4F46E5; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-thought-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-thought-icon { min-width: 24px; min-height: 24px; color: #818CF8; margin-top: 2px; }
    .mobile-thought-title { font-size: 15px; font-weight: bold; color: #818CF8; margin-bottom: 1px; line-height: 1.12; }
    .mobile-thought-desc { font-size: 12.3px; color: #ededed; line-height: 1.36; margin-top: 2px; }
  }`;
    ...
    <motion.div className="relative w-full overflow-hidden mb-6 sm:mb-8 mx-0 sm:mx-4 lg:mx-6 rounded-none sm:rounded-3xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-thought-card">
            <div className="mobile-thought-row"><Brain className="mobile-thought-icon" /><div><div className="mobile-thought-title">Philosophical Depth</div></div></div>
            <div className="mobile-thought-desc"><strong>{philosophicalDepth}/100</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
...
const SmartBackButton = () => {
  const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-thought-provoking-movies'; };
  return (
...
const AuthorCreditSection = () => (
  <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
      <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of profound cinema</p></div></div>
...
const ThoughtProvokingBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/best-thought-provoking-movies" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>10 Best Thought-Provoking Movies</Link>
...
const ThoughtProvokingMoviePage = ({ movie, tmdbData: movieData, sensitiveData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-thought-provoking-movies');
        sessionStorage.setItem('fromCollectionName', '10 Best Thought-Provoking Movies');
    }
  }, []);
  ...
  const collectionSlug = 'best-thought-provoking-movies';
  const collectionShortTag = 'Thought-Provoking'; 
  ...
  if (scenes.length > 0) {
    ...
    cleanSEODesc = `Parents Guide for ${movie.Title} (${currentMovieYear}). Filmiway has identified zero explicit nudity or sexual content throughout the film's entire runtime.`;
  }
  ...
  return (
      <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
          ...
          <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
              ...
              <ThoughtProvokingBreadcrumb movie={movie} />
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="best-thought-provoking-movies" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
...
export default ThoughtProvokingMoviePage;