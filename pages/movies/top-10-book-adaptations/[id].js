// pages/movies/top-10-book-adaptations/[id].js - LITERARY SLATE THEME ✅
// 100% SEO SAFE: Framed around Literary Depth, Page-to-Screen Fidelity, and Storytelling
// SCHEMA: Maximalist (Hidden Literary Depth Score and FAQs for Bots)
...
const COLORS = {
  accent: '#3b82f6', // Slate Blue for Literary Depth
  accentLight: '#93c5fd', 
  bgPrimary: '#000000ff', 
...
const MOVIE_YEARS = {
  'The Shawshank Redemption': '1994', 'The Lord of the Rings: The Return of the King': '2003', 
  "One Flew Over the Cuckoo's Nest": '1975', 'The Green Mile': '1999', 'Trainspotting': '1996', 
  'Room': '2015', 'Brokeback Mountain': '2005', 'The Reader': '2008', 'Carol': '2015', 'Under the Skin': '2013'
};

const MOVIE_DATA_BY_TITLE = {
  'The Shawshank Redemption': { connection: 'Elevated King\'s brief tale into the defining cinematic text on hope and quiet resilience.' },
  'The Lord of the Rings: The Return of the King': { connection: 'Maintained the immense mythological weight of the book while delivering unparalleled blockbuster spectacle.' },
  "One Flew Over the Cuckoo's Nest": { connection: 'Shifted the perspective away from the novel\'s hallucinations to create a brutal, grounded classic of anti-authoritarianism.' },
  'The Green Mile': { connection: 'Transformed Stephen King\'s serialized novel into a sprawling, deeply moving southern gothic epic about miracles in the dark.' },
  'Trainspotting': { connection: 'Turned Irvine Welsh\'s dialect-heavy novel into a kinetic, visually explosive plunge into addiction without moralizing.' },
  'Room': { connection: 'Brilliantly split Emma Donoghue\'s narrative into a claustrophobic thriller followed by the terrifying reality of freedom.' },
  'Brokeback Mountain': { connection: 'Turned a localized short story into a sweeping, universal tragedy of repressed love and societal expectation.' },
  'The Reader': { connection: 'A complex meditation on generational guilt, shame, and the burden of the past adapted from Bernhard Schlink\'s novel.' },
  'Carol': { connection: 'A masterclass in subtext, glances, and exquisite longing, faithfully adapting Patricia Highsmith\'s groundbreaking romance.' },
  'Under the Skin': { connection: 'Stripped Michel Faber\'s sci-fi novel of narrative, leaving a hypnotic, terrifying sensory experience about alienation.' }
};
...
const getJourneyInsight = (title) => {
  ...
  return data?.connection || 'A masterclass in translating the written word into an unforgettable cinematic experience.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  // ✅ CUSTOM METRIC: Literary Depth Score
  const literaryDepthScore = richData?.literaryDepth || 90;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-journey-card { background: linear-gradient(135deg, #0f172a 0%, #0f0f15 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #3b82f6; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-journey-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-journey-icon { min-width: 24px; min-height: 24px; color: #93c5fd; margin-top: 2px; }
    .mobile-journey-title { font-size: 15px; font-weight: bold; color: #93c5fd; margin-bottom: 1px; line-height: 1.12; }
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-journey-card">
            <div className="mobile-journey-row"><BookOpen className="mobile-journey-icon" /><div><div className="mobile-journey-title">Literary Depth</div></div></div>
            <div className="mobile-journey-desc"><strong>{literaryDepthScore}/100</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(59, 130, 246, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(59, 130, 246, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Literary Fidelity</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Literary Depth Score: {literaryDepthScore}/100</p></div>
                </div>
...
const AuthorCreditSection = () => (
  <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
      <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of literary cinema</p></div></div>
...
const BookAdaptationBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/top-10-book-adaptations" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Top 10 Book Adaptations</Link>
...
const BookAdaptationMoviePage = ({ movie, tmdbData: movieData, sensitiveData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'top-10-book-adaptations');
        sessionStorage.setItem('fromCollectionName', 'Top 10 Book Adaptations');
    }
  }, []);
  ...
  const collectionSlug = 'top-10-book-adaptations';
  ...
  return (
      <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
          ...
          <div className="relative z-10 pt-20 sm:pt-24 lg:pt-28">
              ...
              <BookAdaptationBreadcrumb movie={movie} />
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <motion.div id="watch" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
                      <MovieDetailsSection 
                          movie={movie} 
                          fromBookAdaptationCollection={true} 
                          sensitiveData={sensitiveData} 
                      />
                  </motion.div>
                  
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="top-10-book-adaptations" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
...
export default BookAdaptationMoviePage;