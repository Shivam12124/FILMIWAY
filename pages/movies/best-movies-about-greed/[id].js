// pages/movies/best-movies-about-greed/[id].js - EMERALD/GOLD GREED THEME ✅
// 100% SEO SAFE: Framed around Corporate Corruption, Capitalism, and Moral Decay
// SCHEMA: Maximalist (Hidden Greed Index and FAQs for Bots)
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, TrendingUp, DollarSign } from 'lucide-react';
...
const COLORS = {
  accent: '#10B981', // Emerald Green for Money/Wealth
  accentLight: '#34D399', 
  bgPrimary: '#000000ff', 
  bgCard: 'rgba(11, 11, 11, 0.8)',
  textPrimary: '#FFFFFF', textSecondary: '#E5E7EB', textMuted: '#9CA3AF', textDisabled: '#6B7280',
  borderAccent: 'rgba(16, 185, 129, 0.25)', borderLight: 'rgba(55, 65, 81, 0.5)',
};

const MOVIE_YEARS = {
  'There Will Be Blood': '2007', 'The Wolf of Wall Street': '2013', 'Wall Street': '1987', 
  'The Treasure of the Sierra Madre': '1948', 'Casino': '1995', 'The Big Short': '2015', 
  'Uncut Gems': '2019', 'Greed': '1924', 'American Psycho': '2000', 'A Simple Plan': '1998'
};

const MOVIE_DATA_BY_TITLE = {
  'There Will Be Blood': { connection: 'A terrifying study of capitalism stripped of humanity, where oil and blood mix in the relentless pursuit of monopoly.' },
  'The Wolf of Wall Street': { connection: 'An intoxicating, manic comedy exposing the addictive high of corporate fraud and the systems that protect the wealthy.' },
  'Wall Street': { connection: 'The defining cinematic manifesto of 1980s corporate raiding, capturing the ruthless ethos that "greed is good."' },
  'The Treasure of the Sierra Madre': { connection: 'A timeless fable demonstrating how the mere presence of gold breeds violent paranoia and mutual destruction.' },
  'Casino': { connection: 'A neon-lit Greek tragedy where a mafia empire is dismantled from within by hubris, addiction, and unchecked greed.' },
  'The Big Short': { connection: 'A darkly comedic, enraging expose of the institutional fraud that caused the 2008 crash—and the men who profited from it.' },
  'Uncut Gems': { connection: 'A suffocating panic attack following a man who physically cannot stop chasing the high of the next big score.' },
  'Greed': { connection: 'The defining silent film masterpiece showcasing how the obsession with wealth destroys human decency.' },
  'American Psycho': { connection: 'A biting satire connecting 1980s yuppie materialism and superficiality to literal psychopathy and emptiness.' },
  'A Simple Plan': { connection: 'A heartbreaking thriller illustrating how the sudden acquisition of illicit money erodes the morals of ordinary people.' }
};

const getTMDBImage = (path, size = 'w1280') =>
  path ? `https://image.tmdb.org/t/p/${size}${path}` : undefined;

const getGreedInsight = (title) => {
  const data = MOVIE_DATA_BY_TITLE[title];
  return data?.connection || 'A brilliant cinematic exploration of the corrupting power of wealth and unchecked ambition.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const insight = getGreedInsight(movie?.Title);
  // ✅ CUSTOM METRIC: Greed Index
  const greedIndex = richData?.greedIndex || 85;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-greed-card { background: linear-gradient(135deg, #022c22 0%, #0f0f15 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #10B981; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-greed-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-greed-icon { min-width: 24px; min-height: 24px; color: #34D399; margin-top: 2px; }
    .mobile-greed-title { font-size: 15px; font-weight: bold; color: #34D399; margin-bottom: 1px; line-height: 1.12; }
    .mobile-greed-desc { font-size: 12.3px; color: #ededed; line-height: 1.36; margin-top: 2px; }
  }`;
    ...
    <motion.div className="relative w-full overflow-hidden mb-6 sm:mb-8 mx-0 sm:mx-4 lg:mx-6 rounded-none sm:rounded-3xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          <div className="mobile-hero-poster">{posterImage ? <Image src={posterImage} alt={`${movie?.Title} poster`} width={320} height={480} className="w-full h-auto" priority /> : <div style={{ background: COLORS.bgCard, width: '100%', height: '150px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><TrendingUp style={{ color: COLORS.textMuted }} /></div>}</div>
          <div className="mobile-greed-card">
            <div className="mobile-greed-row"><TrendingUp className="mobile-greed-icon" /><div><div className="mobile-greed-title">Greed Index</div></div></div>
            <div className="mobile-greed-desc"><strong>{greedIndex}/100</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 items-start">
            <motion.div className="flex-shrink-0 relative w-24 sm:w-48 md:w-56 lg:w-80 mx-auto sm:mx-0" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.8 }}>
              <div className="relative" style={{ aspectRatio: '2/3' }}>
                {posterImage ? <Image src={posterImage} alt={`${movie?.Title} poster`} fill sizes="(max-width: 640px) 96px, (max-width: 768px) 192px, (max-width: 1024px) 224px, 320px" quality={85} className="object-cover rounded-lg sm:rounded-xl shadow-2xl" /> : <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: COLORS.bgCard, borderRadius: '12px' }}><TrendingUp style={{ color: COLORS.textMuted }} /></div>}
              </div>
            </motion.div>
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(16, 185, 129, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(16, 185, 129, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  <motion.div className="p-2 sm:p-2.5 rounded-lg sm:rounded-xl flex-shrink-0" style={{ background: `linear-gradient(135deg, ${COLORS.accent}20, ${COLORS.accent}10)`, border: `1px solid ${COLORS.accent}40` }} whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }}><DollarSign className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" style={{ color: COLORS.accent }} /></motion.div>
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>The Cost of Ambition</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Greed Index: {greedIndex}/100</p></div>
                </div>
...
const AuthorCreditSection = () => (
  <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
      <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of capitalism in cinema</p></div></div>
...
const GreedBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/best-movies-about-greed" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Movies About Greed</Link>
...
const GreedMoviePage = ({ movie, tmdbData: movieData, sensitiveData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-movies-about-greed');
        sessionStorage.setItem('fromCollectionName', 'Best Movies About Greed');
    }
  }, []);
  ...
  const collectionSlug = 'best-movies-about-greed';
  const collectionShortTag = 'Capitalism'; 
          ...
          <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
              ...
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <motion.div id="watch" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
                      ...
                      <MovieDetailsSection 
                          movie={movie} 
                          fromGreedCollection={true} 
                          sensitiveData={sensitiveData} 
                      />
                  </motion.div>
                  
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="best-movies-about-greed" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
