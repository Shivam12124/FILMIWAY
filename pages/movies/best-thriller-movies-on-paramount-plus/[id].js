// pages/collection/best-thriller-movies-on-paramount-plus/[id].js - PARAMOUNT+ THRILLER MOVIES
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Zap, Brain, Theater, Eye } from 'lucide-react';
...
import { 
  ...
  COMPLETE_MOVIE_DATA,
  SENSITIVE_TIMELINES,
  PARAMOUNT_THRILLER_MOVIE_FAQS 
} from '../../../utils/paramountThrillerMovieData';

// ✅ THEME: Intense Red & Slate
const COLORS = {
  accent: '#EF4444',           // Red
  accentLight: '#F87171',      // Light Red
  bgPrimary: '#000000ff',      // Pure Black
  bgCard: 'rgba(11, 11, 11, 0.8)', // Translucent Slate
  textPrimary: '#FFFFFF', 
  textSecondary: '#E5E7EB',    // Slate-200
  textMuted: '#9CA3AF',        // Slate-400
  textDisabled: '#6B7280',     // Slate-500
...
const MOVIE_DATA_BY_TITLE = {
  'No Country for Old Men': { connection: 'A relentless pursuit across a desolate landscape where fate is decided by a coin toss.' },
  'Zodiac': { connection: 'An obsessive procedural that captures the terrifying uncertainty of hunting a ghost.' },
  '10 Cloverfield Lane': { connection: 'A masterclass in claustrophobic tension where the monster inside might be worse than the one outside.' },
  'Strange Darling': { connection: 'A twisted, non-linear cat-and-mouse game that subverts every expectation of the genre.' },
  'Collateral': { connection: 'A sleek, real-time urban nightmare where a cab ride becomes a philosophical battle for survival.' },
  'A Quiet Place': { connection: 'A nerve-shredding sensory experience where silence is the only way to stay alive.' },
  'Gone Baby Gone': { connection: 'A gritty moral maze that asks impossible questions about justice and the welfare of a child.' },
  'The Autopsy of Jane Doe': { connection: 'A claustrophobic, forensic horror that uncovers terror layer by surgical layer.' },
  'Jack Reacher': { connection: 'A sharp, no-nonsense investigation led by a hero who operates outside the boundaries of the law.' },
  'The Machinist': { connection: 'A haunting psychological descent into guilt, paranoia, and physical decay.' }
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-thriller-movies-on-paramount-plus'; };
    return (
...
const ParamountThrillerBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-thriller-movies-on-paramount-plus" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Thriller Movies on Paramount+</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = PARAMOUNT_THRILLER_MOVIE_FAQS[movie.Title] || [];
  ...
  // 2. METRICS (Using Thriller Specific Terms for Bots)
  ...
  const fullDescription = `
    ${data?.synopsis || movie.description || "A suspenseful thriller film."}
    ...
    Ranking: #${movie.rank || 'N/A'} in Paramount+ Thrillers.
  ...
  const movieSchema = {
    ...
    "genre": data?.dna ? Object.keys(data.dna) : ["Thriller", "Suspense"],
    "keywords": "Thriller Movies Paramount+, Best Suspense Films, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/collection/best-thriller-movies-on-paramount-plus/${movie.imdbID}`, 
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const ParamountThrillerMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-thriller-movies-on-paramount-plus');
        sessionStorage.setItem('fromCollectionName', 'Best Thriller Movies on Paramount+');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/collection/best-thriller-movies-on-paramount-plus/${movie.imdbID}`} />
                ...
                <meta property="og:description" content="A suspenseful thriller on Paramount+." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A suspenseful thriller on Paramount+." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <ParamountThrillerBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing Paramount+ Thriller Context */}
                        <MovieDetailsSection movie={movie} fromParamountThrillerCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-thriller-movies-on-paramount-plus" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default ParamountThrillerMoviePage;