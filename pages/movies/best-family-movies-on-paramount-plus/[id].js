// pages/collection/best-family-movies-on-paramount-plus/[id].js - PARAMOUNT+ FAMILY MOVIES
// VISUALS: Magical/Whimsical Theme (Yellow/Blue Accents)
...
const COLORS = {
  ...
  accentLight: '#FEF08A',      // Light Yellow
...
const MOVIE_DATA_BY_TITLE = {
  'Rango': { connection: 'A visually stunning western with a lizard\'s soul. Gore Verbinski delivers an animated masterpiece.' },
  'TMNT: Mutant Mayhem': { connection: 'A stylish, energetic reboot that captures teenage chaos perfectly with sketchbook-style animation.' },
  'The SpongeBob SquarePants Movie': { connection: 'A surreal, hilarious road trip that defines childhood joy. Pure Goofy Goober energy.' },
  'Sonic the Hedgehog 3': { connection: 'Shadow enters the fray in this high-stakes, emotional sequel that expands the Sonic universe.' },
  'Dora and the Lost City of Gold': { connection: 'A surprisingly funny and heartfelt live-action adaptation that respects the source material.' },
  'Sonic the Hedgehog 2': { connection: 'More action, more Tails, and Jim Carrey at his absolute best. A perfect sequel.' },
  'Hey Arnold! The Jungle Movie': { connection: 'The closure fans waited decades for, delivered with heart, adventure, and nostalgia.' },
  'Sonic the Hedgehog': { connection: 'The origin story that proved video game movies can work. Fast, funny, and full of heart.' },
  'PAW Patrol: The Movie': { connection: 'Bigger action and genuine emotional depth for the preschool heroes in their first theatrical outing.' },
  'PAW Patrol: The Mighty Movie': { connection: 'Superpowers meet super emotions in this fun sequel about confidence and teamwork.' }
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-family-movies-on-paramount-plus'; };
    return (
...
const ParamountFamilyBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-family-movies-on-paramount-plus" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Family Movies on Paramount+</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = PARAMOUNT_FAMILY_MOVIE_FAQS[movie.Title] || [];
  ...
  const fullDescription = `
    ...
    Ranking: #${movie.rank || 'N/A'} in Paramount+ Family Movies.
  ...
  const movieSchema = {
    ...
    "keywords": "Family Movies Paramount+, Kids Movies, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/collection/best-family-movies-on-paramount-plus/${movie.imdbID}`, 
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const ParamountFamilyMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-family-movies-on-paramount-plus');
        sessionStorage.setItem('fromCollectionName', 'Best Family Movies on Paramount+');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/collection/best-family-movies-on-paramount-plus/${movie.imdbID}`} />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing Paramount Family Context */}
                        <MovieDetailsSection movie={movie} fromParamountFamilyCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-family-movies-on-paramount-plus" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default ParamountFamilyMoviePage;