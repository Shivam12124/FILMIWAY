...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Award } from 'lucide-react'; // Added Award icon for Suspense Index
...
const COLORS = {
  accent: '#EAB308', // Amber/Yellow for Thriller
  accentLight: '#FDE047', 
  bgPrimary: '#0B0B0C', 
...
const MOVIE_YEARS = {
  'Se7en': '1995', 'Prisoners': '2013', 'No Country for Old Men': '2007', 'The Silence of the Lambs': '1991',
  'Memories of Murder': '2003', 'Mystic River': '2003', 'The Departed': '2006', 'Wind River': '2017',
  'Gone Girl': '2014', 'The Game': '1997'
};

const MOVIE_DATA_BY_TITLE = {
  'Se7en': { connection: 'A relentlessly dark, grim, and devastatingly intricate psychological thriller. This film stands as a true masterpiece of the detective genre.' },
  'Prisoners': { connection: 'A deeply desperate and morally complex cinematic journey centered on the frantic search for two trapped children.' },
  'No Country for Old Men': { connection: 'A chillingly cold, terrifying, and philosophical masterpiece. This film redefines the Western genre with its stark violence.' },
  'The Silence of the Lambs': { connection: 'A genuine masterclass in psychological chess and riveting suspense. It features a legendary duel of wits.' },
  'Memories of Murder': { connection: 'A landmark Korean crime classic that expertly dissects the systemic flaws within police procedure and the deep, haunting psychological effects.' },
  'Mystic River': { connection: 'An emotionally complex and profoundly moving drama where the indelible trauma of childhood tragedy intersects catastrophically.' },
  'The Departed': { connection: 'A furious, blood-soaked saga of betrayal, chaos, and double-crossing. Scorsese crafts the ultimate modern crime thriller.' },
  'Wind River': { connection: 'A deeply affecting and white-knuckle murder mystery set against the brutally beautiful backdrop of the isolated Wind River Indian Reservation.' },
  'Gone Girl': { connection: 'A twisted examination of marriage, media spectacle, and ruthless manipulation. Fincher crafts a mystery that constantly pulls the rug out.' },
  'The Game': { connection: 'An intense, mind-bending thriller where wealth and reality dissolve into an orchestrated nightmare.' }
};
...
const getThrillerInsight = (title) => {
  ...
  return data?.connection || 'A compelling narrative defined by its intricate plot, masterful character development, and gripping sense of suspense.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const insight = getThrillerInsight(movie?.Title);
  // METRIC CHANGE: Suspense Intensity
  const suspenseIntensity = richData?.suspenseIntensity || 85;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-thriller-card { background: linear-gradient(135deg, #1e1b4b 0%, #0f172a 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #EAB308; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-thriller-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-thriller-icon { min-width: 24px; min-height: 24px; color: #FDE047; margin-top: 2px; }
    .mobile-thriller-title { font-size: 15px; font-weight: bold; color: #FDE047; margin-bottom: 1px; line-height: 1.12; }
    .mobile-thriller-desc { font-size: 12.3px; color: #ededed; line-height: 1.36; margin-top: 2px; }
  }`;
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(234, 179, 8, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(234, 179, 8, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This Thriller is Special</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Suspense Index: {suspenseIntensity}/100</p></div>
                </div>
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-thriller-movies'; };
    return (
...
const AuthorCreditSection = () => (
    <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
            <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of thriller cinema</p></div></div>
...
const ThrillerBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-thriller-movies" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Thriller Movies</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = THRILLER_FAQS[movie.Title] || [];
  ...
  // 2. METRICS (Metrics Changed for Thriller)
  const intensityStats = `
    [FILMIWAY METRICS]
    - Suspense Intensity: ${data?.suspenseIntensity || 0}/100
    - Pacing: ${data?.pacing || 0}/100
    - Plot Twist Impact: ${data?.twistImpact || 0}/100
  `;
  ...
  const fullDescription = `
    ${data?.synopsis || movie.description || "A gripping suspense thriller."}
    ...
    Ranking: #${movie.rank || 'N/A'} in Thriller Cinema.
  ...
  const movieSchema = {
    ...
    "genre": data?.dna ? Object.keys(data.dna) : ["Thriller", "Suspense", "Mystery"],
    "keywords": "Thriller, Suspense, Mystery, Twist Ending, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/movies/thriller/${movie.imdbID}`,
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const ThrillerMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-thriller-movies');
        sessionStorage.setItem('fromCollectionName', 'Best Thriller Movies');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                {/* 🔥 HYDRATION BUG FIX */}
                <title>{`${movie.Title} (${currentMovieYear}) - Best Thriller Movies | Filmiway`}</title>
                <meta name="description" content={`${movie.Title} (${currentMovieYear}) - A gripping suspense thriller. Analysis, intensity ratings & where to stream.`} />
                ...
                {/* Meta Tags */}
                <meta property="og:title" content={`${movie.Title} (${currentMovieYear}) - Suspense Thriller`} />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                {/* ✅ HIDDEN H1 FOR SEO */}
                <h1 className="sr-only">{`${movie.Title} (${currentMovieYear}) - Best Thriller Movies`}</h1>

                <ThrillerBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-thriller-movies" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
