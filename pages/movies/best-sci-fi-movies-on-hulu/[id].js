...
// ✅ MOVIE YEARS FOR HULU SCI-FI (Mapped for Breadcrumbs/SEO)
const MOVIE_YEARS = {
    'Donnie Darko': '2001',
    'The Abyss': '1989',
...
// ✅ MOVIE CONNECTIONS (The Vibe / Editorial Insight)
const MOVIE_DATA_BY_TITLE = {
    'Donnie Darko': { connection: 'The ultimate cult puzzle. It captures the specific dread of being a teenager while navigating time travel, alternate universes, and a giant terrifying rabbit.' },
    'The Abyss': { connection: 'James Cameron’s underwater masterpiece. It combines the claustrophobia of a submarine thriller with a Spielbergian sense of cosmic wonder.' },
    'Dawn of the Planet of the Apes': { connection: 'Shakespeare with monkeys. A stunningly animated war drama that treats its CGI characters with more humanity than most live-action films.' },
    'Predator': { connection: 'The perfect fusion of 80s action and sci-fi horror. It starts as a military mission and seamlessly morphs into a primal slasher movie.' },
    'Mr. Nobody': { connection: 'A kaleidoscope of choices. It visualizes the butterfly effect with dazzling creativity, asking what happens if we refuse to choose a single path.' },
    'No One Will Save You': { connection: 'A masterclass in visual storytelling. With almost no dialogue, it delivers a terrifying, high-energy alien invasion that feels fresh and personal.' },
    'Prey': { connection: 'A feral, stripped-down prequel. It revitalized the franchise by pitting high-tech weaponry against Comanche survival skills in a battle of wits.' },
    'Predator: Killer of Killers': { connection: 'A brutal expansion of the lore. This animated anthology piece brings the Predator to the Viking age with stylized, bloody intensity.' },
    'Rise of the Planet of the Apes': { connection: 'The smartest blockbuster origin story. It makes you root for the downfall of humanity through the eyes of a chimp named Caesar.' },
    'The Assessment': { connection: 'Black Mirror for parents. A tense, uncomfortable look at a future where reproduction is a privilege that must be earned through psychological torture.' }
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
    ...
    // ✅ DATA MAPPING: Use 'mindBendScore' from Hulu Data
    const mindBendScore = richData?.mindBendScore || 80; 
                        ...
                        <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
                            <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(14, 165, 233, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(14, 165, 233, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                                ...
                                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                                    ...
                                    <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Visionary Analysis</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Mind-Bend Score: {mindBendScore}/100</p></div>
                                </div>
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-sci-fi-movies-on-hulu'; };
    return (
...
const HuluSciFiBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-sci-fi-movies-on-hulu" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Sci-Fi Movies on Hulu</Link>
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <meta property="og:description" content="A top-tier science fiction movie on Hulu." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A top-tier science fiction movie on Hulu." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Reusing the standard Details Section, passing Hulu context */}
