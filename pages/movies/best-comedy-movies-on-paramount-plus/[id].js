// pages/collection/best-comedy-movies-on-paramount-plus/[id].js - PARAMOUNT+ COMEDY MOVIES
...
const COLORS = {
  ...
  textPrimary: '#FFFFFF', 
  textSecondary: '#FEF3C7',    // Light Gold Text
  textMuted: '#FDE047',        // Muted Yellow
  textDisabled: '#CA8A04',     // Dark Gold
  borderAccent: 'rgba(250, 204, 21, 0.25)', 
  borderLight: 'rgba(133, 77, 14, 0.5)',
};
...
const MOVIE_DATA_BY_TITLE = {
  'Tropic Thunder': { connection: 'A scathing, brilliant satire of Hollywood that is as smart as it is offensive. It skewers method acting, war movies, and studio greed.' },
  'Anchorman: The Legend of Ron Burgundy': { connection: 'The gold standard of improv-heavy absurdity. It turns the mundane world of 1970s local news into a surreal battleground of egos.' },
  'The Naked Gun: From the Files of Police Squad!': { connection: 'A masterpiece of deadpan slapstick. It delivers more visual gags per minute than almost any other film in history.' },
  'Scary Movie': { connection: 'A fearless parody that defined a generation. It took the slashers of the 90s and turned their tropes into raunchy, memorable comedy gold.' },
  'The Other Guys': { connection: 'A buddy cop movie that hilariously rejects all buddy cop tropes. It finds comedy in forensic accounting and quiet rage.' },
  'School of Rock': { connection: 'Jack Black at his absolute peak. A perfect blend of rock-and-roll rebellion and genuine heart that appeals to every generation.' },
  'The Wolf of Wall Street': { connection: 'A debauched, three-hour adrenaline rush. It turns white-collar crime into a dark comedy of excess that is impossible to look away from.' },
  'Wayne\'s World': { connection: 'The ultimate slacker comedy. It successfully transitioned from SNL sketch to feature film by keeping its heart and its rock-and-roll spirit.' },
  'Planes, Trains and Automobiles': { connection: 'The perfect holiday road trip movie. It balances hysterical disasters with a deeply moving story about loneliness and friendship.' },
  'Trading Places': { connection: 'A sharp, socially conscious satire wrapped in a screwball comedy. It tackles class, race, and greed with wit and Eddie Murphy\'s charm.' }
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  // ✅ UPDATED: Uses 'laughterIndex' for the primary metric
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-comedy-movies-on-paramount-plus'; };
    return (
...
const ParamountComedyBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-comedy-movies-on-paramount-plus" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Comedy Movies on Paramount+</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear, collectionSlug) => {
  ...
  const fullDescription = `
    ...
    Ranking: #${movie.rank || 'N/A'} in Paramount+ Comedy Movies.
...
const ParamountComedyMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-comedy-movies-on-paramount-plus');
        sessionStorage.setItem('fromCollectionName', 'Best Comedy Movies on Paramount+');
    }
  }, []);
        ...
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing Paramount Comedy Context */}
                        <MovieDetailsSection movie={movie} fromParamountComedyCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-comedy-movies-on-paramount-plus" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default ParamountComedyMoviePage;