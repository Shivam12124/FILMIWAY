// pages/movies/war-films/[id].js - H1 SEO FIX + HYDRATION FIX ✅
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Theater, Swords } from 'lucide-react';
...
const MOVIE_YEARS = {
  'Saving Private Ryan': '1998', 'Apocalypse Now': '1979', 'Lawrence of Arabia': '1962',
  'Come and See': '1985', 'Schindler\'s List': '1993', 'Full Metal Jacket': '1987',
  '1917': '2019', 'Das Boot': '1981', 'Platoon': '1986', 'Paths of Glory': '1957'
};

const MOVIE_DATA_BY_TITLE = {
  'Saving Private Ryan': { connection: "Steven Spielberg's visceral masterpiece captures the raw chaos and terror of D-Day. An unforgettable portrayal of combat's psychological toll and the profound bonds forged between soldiers facing death." },
  'Apocalypse Now': { connection: "Francis Ford Coppola's descent into Vietnam madness remains unmatched in its critique of war's dehumanizing effect. A prophetic journey into the abyss of military power and moral collapse." },
  'Lawrence of Arabia': { connection: "David Lean's epic exploration of myth-making in war. A stunning meditation on ambition, identity, and how legends are born from the crucible of conflict." },
  'Come and See': { connection: "A harrowing Soviet masterpiece that refuses to look away from Nazi atrocities. One of cinema's most devastating indictments of war's inhumanity." },
  'Schindler\'s List': { connection: "Spielberg's profound examination of individual heroism against industrial-scale genocide. A testament to the power of conscience in humanity's darkest hour." },
  'Full Metal Jacket': { connection: "Kubrick's unflinching dissection of how war dehumanizes youth. A dark satire exploring the complicity between military culture and moral destruction." },
  '1917': { connection: "A technical marvel that places viewers directly into the trenches. An intimate human story about duty and survival told through continuous cinematic immersion." },
  'Das Boot': { connection: "A claustrophobic masterpiece humanizing those on both sides of conflict. A profound exploration of how war strips away ideology, leaving only fear and survival instinct." },
  'Platoon': { connection: "Stone's semi-autobiographical descent into Vietnam chaos. A visceral account of how war corrupts innocence and forces impossible moral choices." },
  'Paths of Glory': { connection: "Kubrick's damning critique of military hierarchy and institutional injustice. A searing indictment of those who sacrifice expendable soldiers for glory." }
};
...
const getWarFilmInsight = (title) => {
  ...
  return data?.connection || 'A powerful exploration of war\'s psychological and physical toll on humanity. This film refuses to romanticize conflict, instead revealing its true cost.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const warIntensity = richData?.warIntensity || 88;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-war-card { background: linear-gradient(135deg, #2d1415 0%, #0f0f15 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #dc2626; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-war-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-war-icon { min-width: 24px; min-height: 24px; color: #ef4444; margin-top: 2px; }
    .mobile-war-title { font-size: 15px; font-weight: bold; color: #ef4444; margin-bottom: 1px; line-height: 1.12; }
    .mobile-war-desc { font-size: 12.3px; color: #ededed; line-height: 1.36; margin-top: 2px; }
  }`;
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-war-card">
            <div className="mobile-war-row"><Swords className="mobile-war-icon" /><div><div className="mobile-war-title">War Intensity</div></div></div>
            <div className="mobile-war-desc"><strong>{warIntensity}</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(220, 38, 38, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(220, 38, 38, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  <motion.div className="p-2 sm:p-2.5 rounded-lg sm:rounded-xl flex-shrink-0" style={{ background: `linear-gradient(135deg, ${COLORS.accent}20, ${COLORS.accent}10)`, border: `1px solid ${COLORS.accent}40` }} whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }}><Swords className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" style={{ color: COLORS.accent }} /></motion.div>
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This War Film Matters</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>War Intensity Level: {warIntensity}/100</p></div>
                </div>
...
const SmartBackButton = () => {
  const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-war-films'; };
  return (
...
const AuthorCreditSection = () => (
  <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
      <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of war cinema</p></div></div>
...
const WarFilmsBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/best-war-films" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best War Films</Link>
...
const WarFilmMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        ...
        sessionStorage.setItem('fromCollectionName', 'Best War Films');
        // Clear other potential collection state
  ...
  const collectionSlug = 'best-war-films';
  ...
  if (scenes.length > 0) {
    ...
    cleanSEODesc = `${movie.Title} Parents Guide. Filmiway editors have manually verified zero explicit scenes in the full ${currentRuntime} runtime.`;
  }
  ...
  return (
      <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
          ...
          <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
              ...
              <SmartBackButton />
              <WarFilmsBreadcrumb movie={movie} />
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <motion.div id="watch" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
                      <MovieDetailsSection movie={movie} fromWarFilmsCollection={true} />
                  </motion.div>
                  
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="best-war-films" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
