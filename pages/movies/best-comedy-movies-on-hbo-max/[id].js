// pages/collection/best-comedy-movies-on-hbo-max/[id].js - HBO MAX COMEDY MOVIES
// VISUALS: Comedy/Satire Theme (Yellow/Gold/Amber)
...
const MOVIE_DATA_BY_TITLE = {
  'City Lights': { connection: 'The greatest romantic comedy ever made. It proves that laughter serves humanity best when balanced with genuine tragedy.' },
  'The Great Dictator': { connection: 'Comedy weaponized against fascism. Chaplin’s boldest film uses slapstick to dismantle the absurdity of hate.' },
  'Singin\' in the Rain': { connection: 'The gold standard of classical Hollywood comedy. A perfect blend of meta-satire, physical gags, and pure joy.' },
  'This Is Spinal Tap': { connection: 'The film that invented the mockumentary. Its brilliant improvisational structure changed comedy forever.' },
  'When Harry Met Sally...': { connection: 'The definitive script on relationships. It balances sharp wit with emotional truth like no other rom-com.' },
  'Fargo': { connection: 'A masterpiece of dark comedy. It uses "Minnesota Nice" politeness to contrast with horrific violence in hilarious ways.' },
  'Blazing Saddles': { connection: 'Fearless, anarchic satire. It attacks racism and Hollywood tropes with a barrage of gags that still shock today.' },
  'A Hard Day\'s Night': { connection: 'Innovative and effortlessly cool. It captured the anarchic joy of Beatlemania with a fresh, documentary style.' },
  'Barbie': { connection: 'A candy-colored existential crisis. It uses dolls to deliver a sharp, funny satire on gender roles and being human.' },
  'The LEGO Batman Movie': { connection: 'A loving deconstruction of the Batman mythos. It packs more jokes per minute than any live-action superhero film.' }
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-comedy-movies-on-hbo-max'; };
    return (
...
const HboComedyBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-comedy-movies-on-hbo-max" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Comedy Movies on HBO Max</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = HBO_MAX_COMEDY_MOVIE_FAQS[movie.Title] || [];
  ...
  const movieSchema = {
    ...
    "keywords": "Comedy Movies HBO Max, Funniest Movies on HBO Max, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/collection/best-comedy-movies-on-hbo-max/${movie.imdbID}`,
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const HboComedyMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-comedy-movies-on-hbo-max');
        sessionStorage.setItem('fromCollectionName', 'Best Comedy Movies on HBO Max');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/collection/best-comedy-movies-on-hbo-max/${movie.imdbID}`} />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <HboComedyBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing the correct Comedy context prop */}
                        <MovieDetailsSection movie={movie} fromHboMaxComedyCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-comedy-movies-on-hbo-max" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default HboComedyMoviePage;