// pages/movies/best-raunchy-comedy-movies/[id].js - CHAOTIC COMEDY THEME ✅
// 100% SEO SAFE: Framed around Iconic Scenes, Chaos, and Humor
// SCHEMA: Maximalist (Hidden Chaos Score and FAQs for Bots)
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Smile, Flame } from 'lucide-react';
...
const COLORS = {
  accent: '#DB2777', // Vibrant Hot Pink for Comedy/Chaos
  accentLight: '#F472B6', 
  bgPrimary: '#000000ff', 
  bgCard: 'rgba(11, 11, 11, 0.8)',
  textPrimary: '#FFFFFF', textSecondary: '#E5E7EB', textMuted: '#9CA3AF', textDisabled: '#6B7280',
  borderAccent: 'rgba(219, 39, 119, 0.25)', borderLight: 'rgba(55, 65, 81, 0.5)',
};

const MOVIE_YEARS = {
  'American Pie': '1999', 'Superbad': '2007', 'The Hangover': '2009', 'The 40-Year-Old Virgin': '2005',
  'Wedding Crashers': '2005', 'Road Trip': '2000', 'EuroTrip': '2004', 'Bridesmaids': '2011',
  "There's Something About Mary": '1998', 'Harold & Kumar Go to White Castle': '2004'
};

const MOVIE_DATA_BY_TITLE = {
  'American Pie': { connection: 'The foundational text of the modern teen sex comedy, mixing sweet coming-of-age moments with legendary gross-out gags.' },
  'Superbad': { connection: 'Driven by rapid-fire, insanely vulgar dialogue that perfectly captures how teenagers actually talk to each other.' },
  'The Hangover': { connection: 'A brilliantly structured comedy that plays out like a high-stakes detective mystery masked by absurd, R-rated debauchery.' },
  'The 40-Year-Old Virgin': { connection: 'Kicked off the Judd Apatow era, proving that extreme raunchiness could coexist with genuine, heartfelt emotion.' },
  'Wedding Crashers': { connection: 'Bridged the gap between hard-R raunch comedy and genuine romantic comedy, delivering a heartwarming story about commitment.' },
  'Road Trip': { connection: 'A classic, chaotic journey filled with bizarre encounters, extreme humor, and ultimate early-2000s frat-boy energy.' },
  'EuroTrip': { connection: 'A cult classic that dials up international stereotypes and raunch to absolute maximum levels.' },
  'Bridesmaids': { connection: 'Shattered the glass ceiling for female-led comedies, proving women could execute gross-out humor better than the boys.' },
  "There's Something About Mary": { connection: 'A sweet romantic comedy wrapped in some of the most shockingly wince-inducing physical gags in cinema history.' },
  'Harold & Kumar Go to White Castle': { connection: 'Subverts racial stereotypes while delivering an absolutely unhinged, surreal, and hilarious midnight adventure.' }
};
...
const getComedyInsight = (title) => {
  ...
  return data?.connection || 'A legendary R-rated comedy that pushes the boundaries of humor and absurdity.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  // ✅ CUSTOM METRIC: Chaos Score
  const chaosScore = richData?.raunchinessScore || 85;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-comedy-card { background: linear-gradient(135deg, #38081f 0%, #0f0f15 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #DB2777; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-comedy-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-comedy-icon { min-width: 24px; min-height: 24px; color: #F472B6; margin-top: 2px; }
    .mobile-comedy-title { font-size: 15px; font-weight: bold; color: #F472B6; margin-bottom: 1px; line-height: 1.12; }
    .mobile-comedy-desc { font-size: 12.3px; color: #ededed; line-height: 1.36; margin-top: 2px; }
  }`;
    ...
    <motion.div className="relative w-full overflow-hidden mb-6 sm:mb-8 mx-0 sm:mx-4 lg:mx-6 rounded-none sm:rounded-3xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          <div className="mobile-hero-poster">{posterImage ? <Image src={posterImage} alt={`${movie?.Title} poster`} width={320} height={480} className="w-full h-auto" priority /> : <div style={{ background: COLORS.bgCard, width: '100%', height: '150px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Smile style={{ color: COLORS.textMuted }} /></div>}</div>
          <div className="mobile-comedy-card">
            <div className="mobile-comedy-row"><Flame className="mobile-comedy-icon" /><div><div className="mobile-comedy-title">Chaos Level</div></div></div>
            <div className="mobile-comedy-desc"><strong>{chaosScore}/100</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 items-start">
            <motion.div className="flex-shrink-0 relative w-24 sm:w-48 md:w-56 lg:w-80 mx-auto sm:mx-0" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.8 }}>
              <div className="relative" style={{ aspectRatio: '2/3' }}>
                {posterImage ? <Image src={posterImage} alt={`${movie?.Title} poster`} fill sizes="(max-width: 640px) 96px, (max-width: 768px) 192px, (max-width: 1024px) 224px, 320px" quality={85} className="object-cover rounded-lg sm:rounded-xl shadow-2xl" /> : <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: COLORS.bgCard, borderRadius: '12px' }}><Smile style={{ color: COLORS.textMuted }} /></div>}
              </div>
            </motion.div>
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(219, 39, 119, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(219, 39, 119, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This Film is Iconic</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Chaos Level: {chaosScore}/100</p></div>
                </div>
...
const SmartBackButton = () => {
  const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-raunchy-comedy-movies'; };
  return (
...
const AuthorCreditSection = () => (
  <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
      <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of comedic cinema</p></div></div>
...
const ComedyBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/best-raunchy-comedy-movies" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Raunchy Comedy Movies</Link>
...
const RaunchyComedyMoviePage = ({ movie, tmdbData: movieData, sensitiveData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-raunchy-comedy-movies');
        sessionStorage.setItem('fromCollectionName', 'Best Raunchy Comedy Movies');
    }
  }, []);
  ...
  const collectionSlug = 'best-raunchy-comedy-movies';
  const collectionShortTag = 'Chaos'; 
          ...
          <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
              ...
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <motion.div id="watch" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
                      ...
                      <MovieDetailsSection 
                          movie={movie} 
                          fromRaunchyComedyCollection={true} 
                          sensitiveData={sensitiveData} 
                      />
                  </motion.div>
                  
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="best-raunchy-comedy-movies" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
...
export default RaunchyComedyMoviePage;