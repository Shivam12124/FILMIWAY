// pages/movies/best-erotic-romance-movies/[id].js - PASSIONATE ROSE THEME ✅
// 100% SEO SAFE: Framed around Intimacy, Romance, and Emotional Drama
// SCHEMA: Maximalist (Hidden Passion Score and FAQs for Bots)
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Heart } from 'lucide-react';
...
const COLORS = {
  accent: '#be123c', // Deep Rose for Romance
  accentLight: '#fda4af', 
  bgPrimary: '#000000ff', 
  bgCard: 'rgba(11, 11, 11, 0.8)',
  textPrimary: '#FFFFFF', textSecondary: '#E5E7EB', textMuted: '#9CA3AF', textDisabled: '#6B7280',
  borderAccent: 'rgba(190, 18, 60, 0.25)', borderLight: 'rgba(55, 65, 81, 0.5)',
};

const MOVIE_YEARS = {
  'Blue Is the Warmest Color': '2013', '365 Days': '2020', 'The Dreamers': '2003', 'Lady Chatterley\'s Lover': '2022',
  'Last Tango in Paris': '1972', 'Fifty Shades Darker': '2017', 'Fifty Shades of Grey': '2015', 'Fifty Shades Freed': '2018',
  'Secretary': '2002', 'Indecent Proposal': '1993'
};

// 100% SEO-Safe Critical Analysis Descriptions
const MOVIE_DATA_BY_TITLE = {
  'Blue Is the Warmest Color': { connection: 'A raw, incredibly intimate portrayal of first love, passionate connection, and devastating heartbreak.' },
  '365 Days': { connection: 'A glossy, mainstream phenomenon that brings captive romantic fantasy into a high-fashion, luxurious setting.' },
  'The Dreamers': { connection: 'A provocative look at youth rebellion, cinematic obsession, and uninhibited sexual exploration in 1968 Paris.' },
  'Lady Chatterley\'s Lover': { connection: 'A sensual, emotionally grounded take on a classic novel exploring female desire and breaking class boundaries.' },
  'Last Tango in Paris': { connection: 'A highly controversial, devastating exploration of grief masquerading as primal, anonymous lust.' },
  'Fifty Shades Darker': { connection: 'A romantic thriller that shifts the focus toward confronting past demons and attempting vulnerability.' },
  'Fifty Shades of Grey': { connection: 'The pop-culture behemoth that brought BDSM aesthetics into mainstream blockbuster romance.' },
  'Fifty Shades Freed': { connection: 'A glossy wish-fulfillment conclusion focusing on extreme wealth, marriage, and resolving past threats.' },
  'Secretary': { connection: 'A tender, darkly comedic exploration of therapeutic kink as a pathway to self-acceptance and love.' },
  'Indecent Proposal': { connection: 'A devastating psychological examination of what happens when love and trust are treated as a transaction.' }
};
...
const getRomanceInsight = (title) => {
  ...
  return data?.connection || 'A deeply passionate exploration of intimacy, desire, and human connection.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  // ✅ CUSTOM METRIC: Passion Score
  const passionScore = richData?.passionScore || 85;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-tension-card { background: linear-gradient(135deg, #1a050a 0%, #0f0f15 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #be123c; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-tension-card">
            <div className="mobile-tension-row"><Heart className="mobile-tension-icon" /><div><div className="mobile-tension-title">Passion Score</div></div></div>
            <div className="mobile-tension-desc"><strong>{passionScore}/100</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(190, 18, 60, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(190, 18, 60, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Intimacy & Connection</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Passion Score: {passionScore}/100</p></div>
                </div>
...
const AuthorCreditSection = () => (
  <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
      <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of Romantic Drama</p></div></div>
...
const EroticRomanceBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/best-erotic-romance-movies" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Erotic Romance Movies</Link>
...
const EroticRomanceMoviePage = ({ movie, tmdbData: movieData, sensitiveData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-erotic-romance-movies');
  ...
  const collectionSlug = 'best-erotic-romance-movies';
  ...
  return (
      <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
          ...
          <div className="relative z-10 pt-20 sm:pt-24 lg:pt-28">
              ...
              <EroticRomanceBreadcrumb movie={movie} />
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <motion.div id="watch" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
                      <MovieDetailsSection 
                          movie={movie} 
                          fromEroticRomanceCollection={true} 
                          sensitiveData={sensitiveData} 
                      />
                  </motion.div>
                  
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="best-erotic-romance-movies" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
...
export default EroticRomanceMoviePage;