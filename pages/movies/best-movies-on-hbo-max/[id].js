// pages/collection/best-movies-on-hbo-max/[id].js - TOP 10 BEST MOVIES ON HBO MAX
...
const MOVIE_DATA_BY_TITLE = {
  'Seven Samurai': { connection: 'The blueprint for the modern action movie. Kurosawa\'s epic blends tactical realism with deep humanity.' },
    '2001: A Space Odyssey': { connection: 'The most influential sci-fi film of all time. Kubrick\'s visual symphony explores the past and future of humanity.' },
  'The Lord of the Rings: The Return of the King': { connection: 'The greatest fantasy film ever made. A clean sweep of 11 Oscars and the perfect conclusion to the trilogy.' },
  'The Dark Knight': { connection: 'A crime saga that transcends the superhero genre. Heath Ledger\'s performance changed cinema forever.' },
  'Spirited Away': { connection: 'A masterpiece of imagination. Miyazaki\'s hand-drawn world is a stunning meditation on identity and courage.' },
  'Stalker': { connection: 'A philosophical sci-fi journey into the soul. Tarkovsky\'s visual poetry demands patience and rewards it deeply.' },
  'Paris, Texas': { connection: 'A haunting road movie about love and loss. Ry Cooder\'s slide guitar score is as iconic as the visuals.' },
  'Dune: Part Two': { connection: 'A modern sci-fi epic of staggering scale. Villeneuve delivers a visceral war movie that feels ancient and futuristic.' },
  'City Lights': { connection: 'The pinnacle of silent cinema. Chaplin\'s perfect blend of comedy and pathos ends with the most emotional scene in history.' },
  'Casablanca': { connection: 'The ultimate Hollywood romance. A miraculous script, perfect casting, and an ending that defines sacrifice.' },
...
const getMovieInsight = (title) => {
  ...
  return data?.connection || 'A masterpiece of cinema streaming on HBO Max.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const rating = richData?.rating || 8.5; 
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-movies-on-hbo-max'; };
    return (
...
const HboMaxBestBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-movies-on-hbo-max" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Movies on HBO Max</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = HBO_BEST_MOVIE_FAQS[movie.Title] || [];
  ...
  const intensityStats = `
    ...
    - Mastery Score: ${data?.cinematicMastery || 95}/100
  `;
  ...
  const fullDescription = `
    ${data?.synopsis || movie.description || "A top-rated movie streaming on HBO Max."}
    ...
    Ranking: #${movie.rank || 'N/A'} in Best Movies on HBO Max.
  ...
  const movieSchema = {
    ...
    "keywords": "Best Movies HBO Max, Top Rated Films, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/collection/best-movies-on-hbo-max/${movie.imdbID}`,
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const HboMaxBestMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-movies-on-hbo-max');
        sessionStorage.setItem('fromCollectionName', 'Best Movies on HBO Max');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/collection/best-movies-on-hbo-max/${movie.imdbID}`} />
                ...
                <meta property="og:description" content="One of the best movies streaming on HBO Max." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="One of the best movies streaming on HBO Max." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <HboMaxBestBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing flag for HBO Best Collection */}
                        <MovieDetailsSection movie={movie} fromHboMaxBestCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-movies-on-hbo-max" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default HboMaxBestMoviePage;