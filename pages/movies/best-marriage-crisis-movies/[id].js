// pages/movies/best-marriage-crisis-movies/[id].js - SLATE THEME ✅
// 100% SEO SAFE: Framed around Emotional Realism, Relationship Drama, and The End of Romance
// SCHEMA: Maximalist (Hidden Emotional Weight Score and FAQs for Bots)
...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, HeartCrack } from 'lucide-react';
...
const COLORS = {
  accent: '#334155', // Slate/Stormy Blue for emotional weight
  accentLight: '#94a3b8', 
  bgPrimary: '#000000ff', 
  bgCard: 'rgba(11, 11, 11, 0.8)',
  textPrimary: '#FFFFFF', textSecondary: '#E5E7EB', textMuted: '#9CA3AF', textDisabled: '#6B7280',
  borderAccent: 'rgba(51, 65, 85, 0.4)', borderLight: 'rgba(55, 65, 81, 0.5)',
};

const MOVIE_YEARS = {
  'A Separation': '2011', 'Scenes from a Marriage': '1973', 'Marriage Story': '2019', 'Before Midnight': '2013',
  'Kramer vs. Kramer': '1979', 'American Beauty': '1999', 'An Unmarried Woman': '1978', 'Blue Valentine': '2010',
  'Crazy, Stupid, Love.': '2011', 'Revolutionary Road': '2008'
};

// 100% SEO-Safe Critical Analysis Descriptions
const MOVIE_DATA_BY_TITLE = {
  'A Separation': { connection: 'A masterful, tense exploration of class, religion, and the agonizing complexities of divorce.' },
  'Scenes from a Marriage': { connection: 'A grueling, brutally honest portrait of love, resentment, and the impossibility of fully disconnecting.' },
  'Marriage Story': { connection: 'Captures the devastating bureaucratic nightmare of divorce and the lingering affection that survives it.' },
  'Before Midnight': { connection: 'A masterclass in dialogue exploring the resentment, exhaustion, and deep love of long-term partnership.' },
  'Kramer vs. Kramer': { connection: 'A groundbreaking drama that shifted cultural conversations around gender roles, divorce, and parenthood.' },
  'American Beauty': { connection: 'A dark, satirical look at the emptiness of the American Dream and the facade of the perfect suburban marriage.' },
  'An Unmarried Woman': { connection: 'A landmark feminist film exploring the terrifying, exhilarating freedom of sudden independence.' },
  'Blue Valentine': { connection: 'A devastatingly raw portrait of a relationship, shifting between the hope of courtship and the bitter reality of a marriage in freefall.' },
  'Crazy, Stupid, Love.': { connection: 'A sharply written ensemble comedy that explores mid-life reinvention and fighting for a broken marriage.' },
  'Revolutionary Road': { connection: 'A bleak, powerful look at crushed dreams, suburban conformity, and the terror of an ordinary life.' }
};

const getTMDBImage = (path, size = 'w1280') =>
  path ? `https://image.tmdb.org/t/p/${size}${path}` : undefined;

const getCrisisInsight = (title) => {
  const data = MOVIE_DATA_BY_TITLE[title];
  return data?.connection || 'A profound cinematic exploration of the complexities, resentments, and realities of marriage.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  const insight = getCrisisInsight(movie?.Title);
  // ✅ CUSTOM METRIC: Emotional Weight Score
  const emotionalWeight = richData?.emotionalWeight || 85;
  ...
  @media (max-width: 767px) {
    ...
    .mobile-tension-card { background: linear-gradient(135deg, #0f172a 0%, #020617 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #334155; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-tension-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-tension-icon { min-width: 24px; min-height: 24px; color: #94a3b8; margin-top: 2px; }
    .mobile-tension-title { font-size: 15px; font-weight: bold; color: #94a3b8; margin-bottom: 1px; line-height: 1.12; }
    ...
    <motion.div className="relative w-full overflow-hidden mb-6 sm:mb-8 mx-0 sm:mx-4 lg:mx-6 rounded-none sm:rounded-3xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          <div className="mobile-hero-poster">{posterImage ? <Image src={posterImage} alt={`${movie?.Title} poster`} width={320} height={480} className="w-full h-auto" priority /> : <div style={{ background: COLORS.bgCard, width: '100%', height: '150px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><HeartCrack style={{ color: COLORS.textMuted }} /></div>}</div>
          <div className="mobile-tension-card">
            <div className="mobile-tension-row"><HeartCrack className="mobile-tension-icon" /><div><div className="mobile-tension-title">Emotional Weight</div></div></div>
            <div className="mobile-tension-desc"><strong>{emotionalWeight}/100</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 items-start">
            <motion.div className="flex-shrink-0 relative w-24 sm:w-48 md:w-56 lg:w-80 mx-auto sm:mx-0" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.8 }}>
              <div className="relative" style={{ aspectRatio: '2/3' }}>
                {posterImage ? <Image src={posterImage} alt={`${movie?.Title} poster`} fill sizes="(max-width: 640px) 96px, (max-width: 768px) 192px, (max-width: 1024px) 224px, 320px" quality={85} className="object-cover rounded-lg sm:rounded-xl shadow-2xl" /> : <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: COLORS.bgCard, borderRadius: '12px' }}><HeartCrack style={{ color: COLORS.textMuted }} /></div>}
              </div>
            </motion.div>
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(51, 65, 85, 0.3) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(51, 65, 85, 0.15)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  <motion.div className="p-2 sm:p-2.5 rounded-lg sm:rounded-xl flex-shrink-0" style={{ background: `linear-gradient(135deg, ${COLORS.accent}20, ${COLORS.accent}10)`, border: `1px solid ${COLORS.accent}40` }} whileHover={{ scale: 1.05 }} transition={{ duration: 0.2 }}><HeartCrack className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6" style={{ color: COLORS.accentLight }} /></motion.div>
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accentLight }}>The Reality of Romance</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Emotional Weight Score: {emotionalWeight}/100</p></div>
                </div>
...
const AuthorCreditSection = () => (
  <motion.section className="pt-6 sm:pt-8 mt-12 sm:mt-16" style={{ borderTop: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.0, duration: 0.8 }}>
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
      <div className="flex items-center gap-3"><User className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: COLORS.textDisabled }} /><div><p className="text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>Curated by <span className="font-medium" style={{ color: COLORS.textSecondary }}>Filmiway Editorial Team</span></p><p className="text-xs" style={{ color: COLORS.textDisabled }}>Expert analysis of Relationship Drama</p></div></div>
...
const MarriageCrisisBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/best-marriage-crisis-movies" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accentLight} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>10 Best Movies About Marriage Crises</Link>
      <ChevronLeft size={14} className="flex-shrink-0" style={{ color: COLORS.textDisabled, transform: 'rotate(180deg)' }} /><span className="font-medium truncate" style={{ color: `${COLORS.accentLight}B3` }}>{movie.Title}</span>
    </div>
  </motion.nav>
);

const MarriageCrisisMoviePage = ({ movie, tmdbData: movieData, sensitiveData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-marriage-crisis-movies');
        sessionStorage.setItem('fromCollectionName', '10 Best Movies About Marriage Crises');
    }
  }, []);
  ...
  const collectionSlug = 'best-marriage-crisis-movies';
  ...
  return (
      <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
          ...
          <div className="relative z-10 pt-20 sm:pt-24 lg:pt-28">
              ...
              <MarriageCrisisBreadcrumb movie={movie} />
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  <motion.div id="watch" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
                      <MovieDetailsSection 
                          movie={movie} 
                          fromMarriageCrisisCollection={true} 
                          sensitiveData={sensitiveData} 
                      />
                  </motion.div>
                  
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="best-marriage-crisis-movies" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
...
export default MarriageCrisisMoviePage;