// pages/movies/heist-thriller/[id].js
...
// Import heist thriller movie data
...
const getHeistInsight = (title) => {
  switch (title) {
    case 'Heat':
      return 'A masterful cat-and-mouse thriller with breathtaking heist sequences and stellar performances from De Niro and Pacino.';
    case 'Reservoir Dogs':
      return 'A gritty, dialogue-driven thriller about loyalty and betrayal in a failed heist gone catastrophically wrong.';
    case 'The Town':
      return 'A thrilling crime drama where love collides with a life of crime in the tight-knit neighborhoods of Boston.';
    case 'Hell or High Water':
      return 'A modern western thriller filled with emotional depth about brothers planning desperate bank robberies.';
    case 'The Bank Job':
      return 'An intricately woven heist exposing political conspiracies and corruption at the highest levels.';
    case 'Inside Man':
      return 'A clever and stylish bank robbery layered with suspense and unexpected twists from Spike Lee.';
    case 'Thief':
      return 'A gritty neo-noir exploring the professional life of a master safecracker seeking one last big score.';
    case 'Dog Day Afternoon':
      return 'A tense and eccentric hostage drama based on a true story with iconic performances.';
    case 'The Italian Job':
      return 'A stylish, fast-paced heist with iconic Mini Cooper chase scenes through Turin.';
    case "Ocean's Eleven":
      return 'An ensemble caper with wit, charm, and elaborate casino heists executed with style.';
    default:
      return 'An unforgettable heist thriller packed with suspense, action, and masterful execution.';
  }
};
...
const HeistBackButton = () => {
  const handleBackClick = () => {
    if (typeof window !== 'undefined') {
      window.location.href = '/collection/best-heist-thriller-movies';
    }
  };
...
const HeistThrillerMoviePage = ({ movie }) => {
  ...
  return (
    <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
      <Head>
        <title>{movie.title} ({movie.year}) - Heist Thriller | Filmiway</title>
        <meta name="description" content={`${movie.title} (${movie.year}) - ${movie.synopsis?.substring(0, 150) || 'Heist thriller film'}...`} />
        <link rel="canonical" href={`https://filmiway.com/movies/heist-thriller/${movie.imdbID}`} />
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0" />
        <meta name="robots" content="index, follow" />
        <meta property="og:url" content={`https://filmiway.com/movies/heist-thriller/${movie.imdbID}`} />
      ...
      <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
        <OptimizedBanner movie={movie} movieData={mergedMovieData} trailer={trailer} isMobile={isMobile} />
          ...
          <motion.div id="watch" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2, duration: 0.8 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
            <MovieDetailsSection movie={{ ...movie, Title: movie.title }} fromHeistThrillerCollection={true} />
          </motion.div>
        </div>
        ...
        >
          ...
          <div className="flex justify-center items-center gap-8 sm:gap-12 mb-8">
            ...
            <div className="text-yellow-400/60 text-xs tracking-[0.3em] uppercase font-medium">
              Filmiway · Heist Thriller Collection
            </div>
...
export async function getStaticPaths() {
  const paths = HEIST_THRILLER_MOVIES.map((movie) => ({
...
export async function getStaticProps({ params }) {
  const movie = HEIST_THRILLER_MOVIES.find((m) => m.imdbID === params.id);
