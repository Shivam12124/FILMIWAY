// pages/movies/like-memento/[id].js - MOBILE-OPTIMIZED & BOT-READY ✅
// VISUALS: Yellow/Amber Theme (Memory/Polaroid), Minimalist Banner
...
const MOVIE_DATA_BY_TITLE = {
    'Shutter Island': { imdbRating: 8.2, genre: 'Psychological Thriller', director: 'Martin Scorsese', mementoConnection: 'Like Memento, Shutter Island explores unreliable memory and fragmented identity through psychological manipulation.' },
    'Mr. Nobody': { imdbRating: 7.7, genre: 'Sci-Fi Drama', director: 'Jaco Van Dormael', mementoConnection: 'Like Memento, Mr. Nobody questions the nature of memory and identity through non-linear storytelling.' },
    'Primer': { imdbRating: 6.9, genre: 'Sci-Fi Thriller', director: 'Shane Carruth', mementoConnection: 'Like Memento, Primer uses complex timeline structure to create confusion about sequence of events.' },
    'Synecdoche, New York': { imdbRating: 7.5, genre: 'Psychological Drama', director: 'Charlie Kaufman', mementoConnection: 'Like Memento, Synecdoche explores fragmented identity and the unreliability of memory and perception.' },
    'Mulholland Drive': { imdbRating: 7.9, genre: 'Psychological Mystery', director: 'David Lynch', mementoConnection: 'Like Memento, Mulholland Drive uses non-linear narrative to blur the line between reality and delusion.' },
    'Predestination': { imdbRating: 7.4, genre: 'Sci-Fi Thriller', director: 'Michael Spierig, Peter Spierig', mementoConnection: 'Like Memento, Predestination explores identity confusion through a temporal loop structure.' },
    'Coherence': { imdbRating: 7.2, genre: 'Sci-Fi Thriller', director: 'James Ward Byrkit', mementoConnection: 'Like Memento, Coherence creates confusion about reality through memory inconsistencies.' },
    'Donnie Darko': { imdbRating: 8.0, genre: 'Sci-Fi Mystery', director: 'Richard Kelly', mementoConnection: 'Like Memento, Donnie Darko explores mental confusion and the unreliability of perception.' },
    'Enemy': { imdbRating: 6.9, genre: 'Psychological Thriller', director: 'Denis Villeneuve', mementoConnection: 'Like Memento, Enemy explores identity crisis and the fragmentation of self.' },
    'The Fountain': { imdbRating: 7.2, genre: 'Sci-Fi Drama', director: 'Darren Aronofsky', mementoConnection: 'Like Memento, The Fountain deals with memory, loss, and the nature of time and identity.' },
...
// ✅ OPTIMIZED BANNER (Copied Structure from Black Swan)
const OptimizedBanner = ({ movie, movieData, trailer, isMobile }) => {
    ...
    const mementoConnection = correctData?.mementoConnection || `Like Memento, ${movie.Title} explores themes of memory and identity.`;
    
    // Metric for Memento Collection
    const memoryScore = COMPLETE_MOVIE_DATA[movie.tmdbId]?.psychologicalIntensity || 88;
    ...
    @media (max-width: 767px) {
      ...
      .mobile-mem-card { background: linear-gradient(135deg, #1a1600 0%, #0f0f15 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #EAB308; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
      .mobile-mem-row { display: flex; align-items: flex-start; gap: 7px; }
      .mobile-mem-icon { min-width: 24px; min-height: 24px; color: #FDE047; margin-top: 2px; }
      .mobile-mem-title { font-size: 15px; font-weight: bold; color: #FDE047; margin-bottom: 1px; line-height: 1.12; }
      .mobile-mem-desc { font-size: 12.3px; color: #ededed; line-height: 1.36; margin-top: 2px; }
    }`;
        ...
        <motion.div className="relative w-full overflow-hidden mb-6 sm:mb-8 mx-0 sm:mx-4 lg:mx-6 rounded-none sm:rounded-3xl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
                    ...
                    {showTrailer && trailer ? (
                        ...
                        <motion.div key="image" className="absolute inset-0 rounded-none sm:rounded-3xl overflow-hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
                            <div className="relative w-full h-full">
                                {bannerImage ? <Image src={bannerImage} alt={`${movie.Title} banner`} fill priority sizes="100vw" className="object-cover" quality={90} /> : <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: COLORS.bgCard }}><Brain style={{ color: COLORS.textMuted }} /></div>}
            ...
            {isMobile ? (
                <div className="mobile-hero-row">
                    ...
                    <div className="mobile-mem-card">
                        <div className="mobile-mem-row"><Brain className="mobile-mem-icon" /><div><div className="mobile-mem-title">Memory Distortion</div></div></div>
                        <div className="mobile-mem-desc"><strong>{memoryScore}/100</strong> - {mementoConnection.substring(0, 80)}...</div>
                    </div>
                </div>
            ) : (
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 items-start">
                        <motion.div className="flex-shrink-0 relative w-24 sm:w-48 md:w-56 lg:w-80 mx-auto sm:mx-0" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.8 }}>
                            <div className="relative" style={{ aspectRatio: '2/3' }}>
                                {posterImage ? <Image src={posterImage} alt={`${movie?.Title} poster`} fill sizes="(max-width: 640px) 96px, (max-width: 768px) 192px, (max-width: 1024px) 224px, 320px" className="object-cover rounded-lg sm:rounded-xl shadow-2xl" quality={85} /> : <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: COLORS.bgCard, borderRadius: '12px' }}><Brain style={{ color: COLORS.textMuted }} /></div>}
                            </div>
                        </motion.div>
                        <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
                            <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, ${COLORS.bgCard} 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(0, 0, 0, 0.4)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                                ...
                                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                                    ...
                                    <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This Distorts Reality</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Memory Distortion: {memoryScore}/100</p></div>
                                </div>
                                <div className="relative pl-4 sm:pl-6 border-l-2" style={{ borderColor: `${COLORS.accent}40` }}>
                                    ...
                                    <p className="text-xs sm:text-sm lg:text-base xl:text-lg leading-relaxed font-normal break-words" style={{ color: COLORS.textSecondary, lineHeight: '1.8' }}>{mementoConnection}</p>
                                </div>
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/movies-like-memento'; };
    return (
...
const MementoBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/movies-like-memento" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Memento Collection</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const fullDescription = `
    ${data?.synopsis || movie.description || "A compelling memory-loss thriller."}
    
    --- ANALYSIS ---
    [FILMIWAY METRICS] Memory Distortion: ${data?.psychologicalIntensity || 88}/100.
...
const MementoMoviePage = ({ movie }) => {
    ...
    useEffect(() => { if (typeof window !== 'undefined') { sessionStorage.setItem('fromMementoCollection', 'true'); } }, []);
    ...
    const cleanSEODesc = `${movie.Title} (${currentMovieYear}) - A compelling memory-loss thriller like Memento.`;
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="movies-like-memento" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
