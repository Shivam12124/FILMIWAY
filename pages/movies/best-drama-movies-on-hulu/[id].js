// pages/best-drama-movies-on-hulu/[id].js - HULU DRAMA MOVIES
...
// ✅ IMPORT DRAMA DATA (Using Universal Schema Generator)
import {
...
const MOVIE_DATA_BY_TITLE = {
  'Boyhood': { connection: 'A 12-year cinematic experiment capturing the raw, fleeting beauty of growing up in real-time.' },
  'Perfect Days': { connection: 'A meditative masterpiece about finding profound dignity and joy in a simple, structured life.' },
  'Brooklyn': { connection: 'A sweeping, emotionally resonant story of an immigrant torn between two lives and two loves.' },
  'Small Things Like These': { connection: 'A quiet, devastating look at moral courage and complicity in 1980s Ireland.' },
  'All of Us Strangers': { connection: 'A haunting, metaphysical romance exploring grief, loneliness, and the conversations we wish we had.' },
  'BlackBerry': { connection: 'A kinetic, tragicomic rise-and-fall story about tech ambition colliding with human ego.' },
  'A Real Pain': { connection: 'A deeply personal road trip dramedy navigating generational trauma and difficult family bonds.' },
  'Ghostlight': { connection: 'A construction worker finds unexpected healing and emotional release through community theater.' },
  'Little Miss Sunshine': { connection: 'A chaotic, heartfelt family road trip proving that being a "loser" is better than not trying at all.' },
  'R.M.N.': { connection: 'A tense, unflinching examination of xenophobia and social fracture in a small multi-ethnic village.' }
};
...
const SmartBackButton = () => {
  const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-drama-movies-on-hulu'; };
  return (
...
const HuluDramaBreadcrumb = ({ movie }) => (
  <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
    <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
      <Link href="/collection/best-drama-movies-on-hulu" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Drama Movies on Hulu</Link>
...
const HuluDramaMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-drama-movies-on-hulu');
        sessionStorage.setItem('fromCollectionName', 'Best Drama Movies on Hulu');
    }
  }, []);
  ...
  const dynamicCollectionName = (currentSlug || 'best-drama-movies-on-hulu')
    .split('-')
  ...
  const canonicalUrl = `https://filmiway.com/movies/${currentSlug || 'best-drama-movies-on-hulu'}/${movie.imdbID}`;

  const { movieSchema, faqSchema } = generateCleanMovieSchema(
      movie, 
      movieData, 
      currentMovieYear, 
      currentSlug || 'best-drama-movies-on-hulu', 
      'Hulu',
  ...
  return (
      <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
          ...
          <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
              ...
              <HuluDramaBreadcrumb movie={movie} />
              <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                  ...
                  >
                      ...
                      <MovieDetailsSection movie={movie} fromHuluDramaCollection={true} /> 
                  </motion.div>
                  
                  <div className="px-3 sm:px-4 lg:px-6">
                      <InternalCollectionsSection currentSlug="best-drama-movies-on-hulu" />
                      <TMDBAttribution />
                      <AuthorCreditSection />
                  </div>
              </div>
...
export default HuluDramaMoviePage;