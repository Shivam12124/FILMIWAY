...
import { ChevronLeft, Play, X, User, Twitter, Hash, Send, Film, Award } from 'lucide-react'; // Added Award icon for Revenge Index
...
const MOVIE_YEARS = {
  'Oldboy': '2003', 'Gladiator': '2000', 'Unforgiven': '1992', 'Braveheart': '1995',
  'The Prestige': '2006', 'The Revenant': '2015', 'Kill Bill: Vol. 2': '2004', 'John Wick': '2014',
  'I Saw the Devil': '2010', 'The Count of Monte Cristo': '2002'
};

const MOVIE_DATA_BY_TITLE = {
  'Oldboy': { connection: 'A masterpiece of psychological revenge cinema. This Korean classic presents a deeply disturbing mystery wrapped in relentless brutality.' },
  'Gladiator': { connection: 'An epic tale of a fallen general who transforms from slave to arena champion. The film captures the grandeur of ancient Rome.' },
  'Unforgiven': { connection: 'A deconstructive Western masterpiece that strips away Hollywood myths to reveal the true, unglamorous cost of violence.' },
  'Braveheart': { connection: 'A sweeping historical epic of personal betrayal transformed into national liberation. Wallace\'s journey represents vengeance on an epic scale.' },
  'The Prestige': { connection: 'A brilliantly crafted rivalry between two magicians consumed by obsession and revenge. Every meticulous detail serves the shocking reveal.' },
  'The Revenant': { connection: 'An absolutely brutal survival epic where revenge fuels an impossible 200-mile journey through unforgiving wilderness.' },
  'Kill Bill: Vol. 2': { connection: 'Tarantino\'s completion of The Bride\'s roaring rampage of revenge. The second volume adds emotional depth and shocking revelations.' },
  'John Wick': { connection: 'A modern action masterpiece that proves vengeance can fuel elegantly choreographed violence. Keanu Reeves\' understated performance masks a relentless rage.' },
  'I Saw the Devil': { connection: 'An unflinching Korean extreme revenge thriller exploring moral degradation and the corruption that vengeance inflicts on the righteous.' },
  'The Count of Monte Cristo': { connection: 'The quintessential revenge narrative of wrongful imprisonment and meticulous comeuppance. The transformation perfectly embodies slow-burn satisfaction.' }
};
...
const getRevengeInsight = (title) => {
  ...
  return data?.connection || 'A compelling revenge narrative defined by its character depth, thematic complexity, and the psychological journey toward vengeance.';
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
  ...
  @media (max-width: 767px) {
    ...
    .mobile-revenge-card { background: linear-gradient(135deg, #1e1b4b 0%, #0f172a 100%); border-radius: 12px; box-shadow: 0 2px 12px #0006; margin: 0; flex: 1; border-left: 4px solid #EF4444; display: flex; flex-direction: column; justify-content: flex-start; padding: 10px 10px 10px 12px; min-height: 110px; position: relative; }
    .mobile-revenge-row { display: flex; align-items: flex-start; gap: 7px; }
    .mobile-revenge-icon { min-width: 24px; min-height: 24px; color: #F87171; margin-top: 2px; }
      ...
      {isMobile ? (
        <div className="mobile-hero-row">
          ...
          <div className="mobile-revenge-card">
            ...
            <div className="mobile-revenge-desc"><strong>{revengeIndex}</strong> - {insight.substring(0, 80)}...</div>
          </div>
        </div>
      ) : (
            ...
            <motion.div className="flex-1 w-full min-w-0" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.0, duration: 0.8 }}>
              <motion.div className="relative rounded-xl sm:rounded-2xl overflow-hidden p-4 sm:p-6 lg:p-8 backdrop-blur-sm" style={{ background: `linear-gradient(135deg, rgba(239, 68, 68, 0.15) 0%, rgba(15, 15, 20, 0.5) 100%)`, border: `1px solid ${COLORS.borderLight}`, boxShadow: `0 8px 32px rgba(239, 68, 68, 0.2)` }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.1, duration: 0.8 }}>
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1"><h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>Why This Revenge is Essential</h2><p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>Revenge Intensity: {revengeIndex}/100</p></div>
                </div>
...
const RevengeBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-revenge-movies" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Revenge Movies</Link>
...
// ✅ JSON-LD SCHEMA GENERATOR - REVENGE EDITION
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const intensityStats = `
    ...
    - Moral Complexity: ${data?.moralComplexity || 0}/100
  ...
  const movieSchema = {
    ...
    "genre": data?.dna ? Object.keys(data.dna) : ["Action", "Thriller", "Drama"],
    "keywords": "Revenge, Retribution, Justice, Vengeance, " + (data?.themes ? data.themes.join(", ") : ""),
...
const RevengeMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-revenge-movies');
        sessionStorage.setItem('fromCollectionName', 'Best Revenge Movies');
    }
  }, []);
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    <motion.div id="watch" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 2.0, duration: 0.8 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
                        <MovieDetailsSection movie={movie} fromRevengeCollection={true} />
                    </motion.div>
