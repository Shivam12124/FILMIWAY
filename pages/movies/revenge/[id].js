...
// Import revenge movie data
import { REVENGE_MOVIES } from '../../../utils/revengeMovieData';

const COLORS = {
  accent: '#EF4444', accentLight: '#F87171', bgPrimary: '#0B0B0C', bgCard: 'rgba(55, 65, 81, 0.3)',
  textPrimary: '#FFFFFF', textSecondary: '#D1D5DB', textMuted: '#9CA3AF', textDisabled: '#6B7280',
  borderAccent: 'rgba(239, 68, 68, 0.2)', borderLight: 'rgba(107, 114, 128, 0.2)',
};
  ...
  <div className="mobile-hero-row">
    ...
    <div className="mobile-revenge-card">
      <div className="mobile-revenge-row">
        <Award className="mobile-revenge-icon" />
        <div>
          <div className="mobile-revenge-title">Revenge Index</div>
        </div>
      </div>
      <div className="mobile-revenge-desc">
        <strong>{movie?.revengeIntensity || 95}</strong> - {insight}
      </div>
    </div>
  </div>
) : (
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 lg:gap-8 items-start">
            ...
            >
              <motion.div
                ...
                style={{
                  background: `linear-gradient(135deg, ${COLORS.bgCard} 0%, rgba(50, 30, 30, 0.5) 100%)`,
                ...
                <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-5">
                  ...
                  <div className="min-w-0 flex-1">
                    <h2 className="text-sm sm:text-base lg:text-xl xl:text-2xl font-bold leading-tight" style={{ color: COLORS.accent }}>
                      Why This Revenge Film is Essential
                    </h2>
                    <p className="text-xs sm:text-sm hidden sm:block" style={{ color: COLORS.textMuted }}>
                      Revenge Intensity: {movie?.revengeIntensity || 95}/100
                    </p>
                  </div>
                </div>
...
const RevengeBackButton = () => {
  const handleBackClick = () => {
    if (typeof window !== 'undefined') {
      window.location.href = '/collection/best-revenge-movies';
    }
  };
...
const RevengeMoviePage = ({ movie }) => {
  ...
  return (
    <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
      <Head>
        <title>{movie.title} ({movie.year}) - Revenge Film | Filmiway</title>
        <meta name="description" content={`${movie.title} (${movie.year}) - ${movie.synopsis?.substring(0, 150) || 'Revenge thriller film'}...`} />
        <link rel="canonical" href={`https://filmiway.com/movies/revenge/${movie.imdbID}`} />
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0" />
        <meta name="robots" content="index, follow" />
        <meta property="og:url" content={`https://filmiway.com/movies/revenge/${movie.imdbID}`} />
      ...
      <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
        ...
        <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
          <motion.div id="watch" initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2, duration: 0.8 }} className="space-y-8 sm:space-y-12 px-3 sm:px-4 lg:px-6">
            <MovieDetailsSection movie={{ ...movie, Title: movie.title }} fromRevengeCollection={true} />
          </motion.div>
        </div>
        ...
        >
          ...
          <div className="flex justify-center items-center gap-8 sm:gap-12 mb-8">
            ...
            <div className="text-red-400/60 text-xs tracking-[0.3em] uppercase font-medium">
              Filmiway · Revenge Films Collection
            </div>
...
export async function getStaticPaths() {
  const paths = REVENGE_MOVIES.map((movie) => ({
