// pages/collection/best-thriller-movies-on-peacock/[id].js
// PEACOCK THRILLER MOVIES COLLECTION
...
// ✅ IMPORT PEACOCK THRILLER DATA
import { 
    COMPLETE_MOVIE_DATABASE, 
    COMPLETE_MOVIE_DATA,
    SENSITIVE_TIMELINES,
    PEACOCK_THRILLER_MOVIE_FAQS 
...
// ✅ UPDATED MOVIE YEARS FOR PEACOCK THRILLER
const MOVIE_YEARS = {
    'Memento': '2000',
    'The Game': '1997',
    'Sicario': '2015',
...
const MOVIE_DATA_BY_TITLE = {
    'Memento': { connection: 'Christopher Nolan’s reverse-chronological masterpiece that forces you to feel the protagonist’s confusion.' },
    'The Game': { connection: 'A paranoid nightmare where reality and simulation blur until the very last frame.' },
    'Sicario': { connection: 'A suffocating dive into the moral void of the drug war, driven by Roger Deakins\' legendary cinematography.' },
    'Coherence': { connection: 'A masterclass in low-budget sci-fi that turns a dinner party into a quantum physics nightmare.' },
    'Inside Man': { connection: 'Spike Lee’s slick, intelligent heist film that focuses more on the mind games than the money.' },
    'Law Abiding Citizen': { connection: 'A brutal revenge thriller that questions the justice system through a battle of wits and violence.' },
    'The Black Phone': { connection: 'A supernatural survival thriller that grounds its horror in a terrifyingly realistic 1970s atmosphere.' },
    'Triangle': { connection: 'A mind-bending time loop horror that punishes its protagonist—and the audience—with relentless déjà vu.' },
    'Limitless': { connection: 'A stylish, high-energy visualization of ultimate potential and the dangerous cost of a shortcut to success.' },
    'Twisters': { connection: 'A modern disaster spectacle that revitalizes the genre with terrifyingly realistic weather effects.' }
};
...
const OptimizedBanner = ({ movie, movieData, trailer, isMobile, richData }) => {
    ...
    // ✅ UPDATED: Uses 'suspenseIntensity'
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-thriller-movies-on-peacock'; };
    return (
...
const PeacockThrillerBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-thriller-movies-on-peacock" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Thriller Movies on Peacock</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const fullDescription = `
    ...
    Ranking: #${movie.rank || 'N/A'} in Peacock Thriller Movies.
...
const PeacockThrillerMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-thriller-movies-on-peacock');
        sessionStorage.setItem('fromCollectionName', 'Best Thriller Movies on Peacock');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <meta property="og:description" content="A gripping thriller movie on Peacock." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A gripping thriller movie on Peacock." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <PeacockThrillerBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing Peacock Thriller Context */}
                        <MovieDetailsSection movie={movie} fromPeacockThrillerCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        {/* ✅ Updated Slug */}
                        <InternalCollectionsSection currentSlug="best-thriller-movies-on-peacock" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default PeacockThrillerMoviePage;