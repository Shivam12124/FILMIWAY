// pages/best-movies-on-peacock/[id].js - TOP 10 BEST MOVIES ON PEACOCK
...
// ✅ UPDATED MOVIE YEARS FOR PEACOCK LIST
const MOVIE_YEARS = {
  'Memento': '2000', 
  'Sicario': '2015',
  'The Revenant': '2015', 
...
const MOVIE_DATA_BY_TITLE = {
  'Memento': { connection: 'Nolan’s structural masterpiece. A reverse-chronology noir that demands your absolute attention.' },
  'Sicario': { connection: 'The drug war stripped of glory. Villeneuve builds unbearable tension in every single frame.' },
  'The Revenant': { connection: 'Visceral survival. DiCaprio vs. Nature in a film shot entirely in natural light.' },
  'Requiem for a Dream': { connection: 'A harrowing descent. The most stylish and devastating look at addiction ever filmed.' },
  'Man on Fire': { connection: 'Denzel at his most lethal. A revenge thriller with genuine emotional weight and style.' },
  'Apocalypto': { connection: 'Pure kinetic energy. A relentless chase through the Mayan jungle that never slows down.' },
  'About Time': { connection: 'Time travel with a heart. A rom-com that teaches you to appreciate the ordinary days.' },
  'Coherence': { connection: 'Sci-fi on a budget. A dinner party goes wrong in a mind-bending quantum puzzle.' },
  'Harry Potter and the Prisoner of Azkaban': { connection: 'The franchise turning point. Cuarón brings darkness, style, and maturity to Hogwarts.' },
  'Monty Python and the Holy Grail': { connection: 'Comedy royalty. The most quotable, absurdist medieval satire in history.' }
};
...
const getMovieInsight = (title) => {
  ...
  return data?.connection || 'A masterpiece of cinema streaming on Peacock.';
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-movies-on-peacock'; };
    return (
...
const PeacockBestBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-movies-on-peacock" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Movies on Peacock</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = PEACOCK_BEST_MOVIE_FAQS[movie.Title] || [];
  ...
  const fullDescription = `
    ${data?.synopsis || movie.description || "A top-rated movie streaming on Peacock."}
    ...
    Ranking: #${movie.rank || 'N/A'} in Best Movies on Peacock.
  ...
  const movieSchema = {
    ...
    "keywords": "Best Movies Peacock, Top Rated Films, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/movies/best-movies-on-peacock/${movie.imdbID}`,
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const PeacockBestMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-movies-on-peacock');
        sessionStorage.setItem('fromCollectionName', 'Best Movies on Peacock');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/collection/best-movies-on-peacock/${movie.imdbID}`} />
                ...
                <meta property="og:description" content="One of the best movies streaming on Peacock." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="One of the best movies streaming on Peacock." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <PeacockBestBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing Peacock collection prop */}
                        <MovieDetailsSection movie={movie} fromPeacockBestCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-movies-on-peacock" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default PeacockBestMoviePage;