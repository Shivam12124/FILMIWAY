// pages/best-family-movies-on-hulu/[id].js - HULU FAMILY MOVIES
...
const COLORS = {
  accent: '#FACC15', accentLight: '#FEF08A', bgPrimary: '#0f172a', bgCard: 'rgba(15, 23, 42, 0.8)', // Yellow/Slate
...
const MOVIE_DATA_BY_TITLE = {
  'Robot Dreams': { connection: 'A silent, deeply emotional masterpiece that works for kids and adults alike. Gentle, funny, and profoundly moving.' },
  'Night at the Museum': { connection: 'One of the most imaginative family adventures ever made. Packed with humor, wonder, and iconic characters.' },
  'Sonic the Hedgehog 2': { connection: 'Fast, colorful, genuinely funny, and perfect modern family entertainment. Jim Carrey at full comic power.' },
  'Steven Universe: The Movie': { connection: 'Beautiful music, emotional maturity, and one of the best animated stories about growth and healing.' },
  'Hotel Transylvania': { connection: 'Classic monster comedy with heart, humor, and huge rewatch value. Monster fun with surprisingly warm family themes.' },
  'Hotel Transylvania 2': { connection: 'Even funnier than the first, with great family messages about acceptance and parenting. Big laughs, bigger heart.' },
  'The Amazing Maurice': { connection: 'Sharp writing, witty humor, and intelligent storytelling based on Terry Pratchett. Smart family animation.' },
  'My Friend the Penguin': { connection: 'True story, gentle pacing, and pure warmth — ideal for younger kids and families. Real-life magic, simple and beautiful.' },
  'Fireheart': { connection: 'Inspiring, energetic, and full of positive messages about courage and equality. A modern family hero story.' },
  'Oz the Great and Powerful': { connection: 'Visually spectacular fantasy with classic Disney adventure vibes. Pure fantasy spectacle for the whole family.' }
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-family-movies-on-hulu'; };
    return (
...
const HuluFamilyBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-family-movies-on-hulu" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Family Movies on Hulu</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = HULU_FAMILY_MOVIE_FAQS[movie.Title] || [];
  ...
  const fullDescription = `
    ...
    Ranking: #${movie.rank || 'N/A'} in Hulu Family Movies.
  ...
  const movieSchema = {
    ...
    "keywords": "Family Movies Hulu, Kids Movies, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/movies/best-family-movies-on-hulu/${movie.imdbID}`, 
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const HuluFamilyMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-family-movies-on-hulu');
        sessionStorage.setItem('fromCollectionName', 'Best Family Movies on Hulu');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/movies/best-family-movies-on-hulu/${movie.imdbID}`} />
                ...
                <meta property="og:description" content="A great family movie on Hulu." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A great family movie on Hulu." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <HuluFamilyBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Passing Hulu Family Context */}
                        <MovieDetailsSection movie={movie} fromHuluFamilyCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-family-movies-on-hulu" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default HuluFamilyMoviePage;