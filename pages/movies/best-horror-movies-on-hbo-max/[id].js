// pages/movies/best-horror-movies-on-hbo-max/[id].js - HBO MAX HORROR MOVIES
...
// ✅ IMPORT HBO MAX HORROR DATA
import { 
  COMPLETE_MOVIE_DATABASE, 
  COMPLETE_MOVIE_DATA,
  SENSITIVE_TIMELINES,
  HBO_HORROR_MOVIE_FAQS 
} from '../../../utils/hboMaxHorrorMovieData';

const COLORS = {
  accent: '#DC2626', accentLight: '#FCA5A5', bgPrimary: '#000000ff', bgCard: 'rgba(11, 11, 11, 0.8)',// Blood Red/Black
...
const MOVIE_DATA_BY_TITLE = {
  'The Conjuring': { connection: 'The gold standard of modern supernatural horror. James Wan redefined the haunted house genre with pure, escalating dread.' },
  'Insidious': { connection: 'A masterclass in jump scares and atmosphere. It introduced "The Further" and gave us the iconic Lipstick-Face Demon.' },
  'The Conjuring 2': { connection: 'A rare sequel that matches the original. The crooked man and Valak the Nun are nightmare fuel.' },
  'Annabelle: Creation': { connection: 'A prequel that surpasses its predecessor. It turns a simple doll origin story into a relentless terrifying ride.' },
  'The Witch': { connection: 'A slow-burn masterpiece of folk horror. It builds an atmosphere of suffocating dread and religious hysteria.' },
  'Evil Dead Rise': { connection: 'A blood-soaked, relentless assault on the senses. It moves the franchise to the city without losing its gruesome edge.' },
  'It': { connection: 'The definitive Stephen King adaptation. Pennywise is terrifying, but the heart of the film lies in the bond of the Losers Club.' },
  'The Nun': { connection: 'Gothic horror at its finest. The atmosphere of the Romanian abbey is suffocating, and Valak remains a terrifying villain.' },
  'Annabelle Comes Home': { connection: 'A horror funhouse ride. It unleashes the entire Warren artifact room for a night of varied and creative scares.' },
  'Insidious: Chapter 2': { connection: 'A direct continuation that deepens the lore. It explores the terrifying history of the Bride in Black.' }
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-horror-movies-on-hbo-max'; };
    return (
...
const HboMaxHorrorBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-horror-movies-on-hbo-max" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Horror Movies on HBO Max</Link>
...
const generateMovieSchema = (movie, movieData, currentMovieYear) => {
  ...
  const faqs = HBO_HORROR_MOVIE_FAQS[movie.Title] || [];
  ...
  const fullDescription = `
    ...
    Ranking: #${movie.rank || 'N/A'} in HBO Max Horror Movies.
  ...
  const movieSchema = {
    ...
    "keywords": "Horror Movies HBO Max, Scariest Movies on HBO Max, " + (data?.themes ? data.themes.join(", ") : ""),
    "url": `https://filmiway.com/collection/best-horror-movies-on-hbo-max/${movie.imdbID}`,
    "author": {
      "@type": "Organization",
      "name": "Filmiway",
...
const HboMaxHorrorMoviePage = ({ movie, tmdbData: movieData }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
        sessionStorage.setItem('fromCollection', 'best-horror-movies-on-hbo-max');
        sessionStorage.setItem('fromCollectionName', 'Best Horror Movies on HBO Max');
    }
  }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <link rel="canonical" href={`https://filmiway.com/collection/best-horror-movies-on-hbo-max/${movie.imdbID}`} />
                ...
                <meta property="og:description" content="A terrifying horror movie on HBO Max." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A terrifying horror movie on HBO Max." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <HboMaxHorrorBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        {/* ✅ Note: We pass 'fromHboMaxHorrorCollection' flag here */}
                        <MovieDetailsSection movie={movie} fromHboMaxHorrorCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-horror-movies-on-hbo-max" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default HboMaxHorrorMoviePage;