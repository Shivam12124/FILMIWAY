// pages/collection/best-romance-movies-on-paramount-plus/[id].js
// PARAMOUNT+ ROMANCE MOVIES COLLECTION
...
// ✅ UPDATED MOVIE YEARS FOR PARAMOUNT ROMANCE
const MOVIE_YEARS = {
    'Titanic': '1997',
    'About Time': '2013',
...
// ✅ UPDATED MOVIE INSIGHTS FOR PARAMOUNT ROMANCE
const MOVIE_DATA_BY_TITLE = {
    'Titanic': { connection: 'The ultimate definition of sacrifice. It remains the modern benchmark for epic tragic romance.' },
    ...
    "It's a Wonderful Life": { connection: 'The George–Mary relationship is profound, tender, and the emotional foundation of a life well-lived.' },
    'The English Patient': { connection: 'Grand, poetic, and devastating. A forbidden war romance that explores the boundaries of nations and love.' },
    'The Curious Case of Benjamin Button': { connection: 'A beautiful, original, and bittersweet meditation on two souls meeting in the middle of time.' },
    ...
    "Breakfast at Tiffany's": { connection: 'Timeless style masking a story about two lost souls finding where they belong in New York.' },
    'Like Crazy': { connection: 'Modern, painfully realistic. It captures the agony of long-distance love and bureaucratic fragility.' },
    '50 First Dates': { connection: 'High-concept heart. It redefines devotion as a daily choice to win someone over, again and again.' },
    'Clueless': { connection: 'Teen romance with a satirical edge. Updates Jane Austen for the 90s with wit, style, and heart.' }
};
...
const SmartBackButton = () => {
    const handleBackClick = () => { if (typeof window !== 'undefined') window.location.href = '/collection/best-romance-movies-on-paramount-plus'; };
    return (
...
const ParamountRomanceBreadcrumb = ({ movie }) => (
    <motion.nav className="mb-6 sm:mb-8 px-3 sm:px-4 lg:px-6 pb-3 sm:pb-4" style={{ borderBottom: `1px solid ${COLORS.borderLight}` }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
        <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm" style={{ color: COLORS.textMuted }}>
            <Link href="/collection/best-romance-movies-on-paramount-plus" className="transition-all duration-300 truncate" style={{ color: COLORS.textMuted }} onMouseEnter={(e) => e.currentTarget.style.color = COLORS.accent} onMouseLeave={(e) => e.currentTarget.style.color = COLORS.textMuted}>Best Romance Movies on Paramount+</Link>
...
const ParamountRomanceMoviePage = ({ movie, tmdbData: movieData }) => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            sessionStorage.setItem('fromCollection', 'best-romance-movies-on-paramount-plus');
            sessionStorage.setItem('fromCollectionName', 'Best Romance Movies on Paramount+');
        }
    }, []);
    ...
    return (
        <div className="min-h-screen text-white relative overflow-hidden" style={{ backgroundColor: COLORS.bgPrimary }}>
            <Head>
                ...
                <meta property="og:description" content="A deeply emotional romance movie on Paramount+." />
                <meta property="og:type" content="video.movie" />
                <meta property="og:image" content={movieData?.poster_path ? `https://image.tmdb.org/t/p/w500${movieData.poster_path}` : ''} />
                <meta name="twitter:card" content="summary_large_image" />
                <meta name="twitter:title" content={cleanSEOTitle} />
                <meta name="twitter:description" content="A deeply emotional romance movie on Paramount+." />
            ...
            <div className="relative z-10 pt-10 sm:pt-12 lg:pt-16">
                ...
                <ParamountRomanceBreadcrumb movie={movie} />
                <div className="container mx-auto px-0 pb-16 sm:pb-24 lg:pb-32 max-w-7xl">
                    ...
                    >
                        <MovieDetailsSection movie={movie} fromParamountRomanceCollection={true} /> 
                    </motion.div>
                    
                    <div className="px-3 sm:px-4 lg:px-6">
                        <InternalCollectionsSection currentSlug="best-romance-movies-on-paramount-plus" />
                        <TMDBAttribution />
                        <AuthorCreditSection />
                    </div>
                </div>
...
export default ParamountRomanceMoviePage;